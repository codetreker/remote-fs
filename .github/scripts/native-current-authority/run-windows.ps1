...
$cache = Join-Path $root 'qemu-cache'
$package = Join-Path $cache 'qemu-11.1.0'
...
$archive = Join-Path $cache 'qemu-w64-setup-20260811.exe'
$archiveHash = '5bcf9eed634e8575a37b74f445af41a2fe4106da512d0c30c368301d4c105037fdfab40a5287367a28a957624cddebbc8c07e16c88ab6634f554cdf3d16bf543'
try {
    if (-not (Test-Path -LiteralPath $archive)) {
        $partial = Join-Path $bootstrap 'qemu-download.partial'
        Invoke-WebRequest -Uri 'https://qemu.weilnetz.de/w64/2026/qemu-w64-setup-20260811.exe' -OutFile $partial -TimeoutSec 180 -MaximumRetryCount 1 -RetryIntervalSec 2
    ...
    if (-not (Test-Path -LiteralPath $package)) {
        ...
        if ($executables.Count -ne 1 -or $biosFiles.Count -ne 1) { throw 'Archive must contain one x64 QEMU executable and one BIOS data tree.' }
        $files = [ordered]@{}
