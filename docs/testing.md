...
WNetAddConnection4W 只从 system MPR 解析，精确七参数、DISK NETRESOURCEW、NULL auth/length0、CONNECT_REQUIRE_INTEGRITY-only 及唯一 SDK transport option 分别验证。64-bit NETRESOURCEW 为 48 字节/alignment8；24 字节 option 的 header8、Wsk、TCP port/flag 与所有保留/padding/QUIC/RDMA/skip 零值按 host ABI 核对，UTF-16/pointer 在同步调用返回前存活。删除/改变 port 或加入禁用 flags 的对照必须命中断言。此数据结构与 WNet4 provider-specific buffer 的配对尚未原生证明；不可用 export/不支持选项/认证/签名/路由失败保持失败，不换 API、PowerShell/CIM/CLI、445 或其它策略。
