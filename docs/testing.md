...
本地可移植 mapping 23 根/67 verdict、进程拥有者 7 根/26 verdict 的普通/race 检查通过；忽略 resume count、重复关闭 stdin 的两个隔离对照分别命中断言。五项 ARM64 程序/默认与 tagged 测试构建、vet/格式检查和十七项 Python checker 用例通过；可移植控制的 verdict 不包括 Windows 适配或 tagged 原生单元。新原生四单元尚未运行，这些本地控制不验证 Windows PowerShell 启动、映射、guest 或缓存，也不宣称修复四秒超时。
