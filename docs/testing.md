...
诊断的非拥有者 adapter 在每次 delegated Start 观察 PSModulePath、WinPSModulePath、PSModuleAnalysisCachePath 后只调用原拥有者一次，不改变环境或启动参数。每项 presence、UTF-8 长度、SHA256、至多 4096 字节的 base64 前缀及截断单独记录；缺席值和未到 Start 分开，原 NUL cache scope 保持。仅 tagged 诊断 receipt 允许至多 256 KiB，原命令捕获/执行限额不变。这些后续环境事实不能重建原 cold 环境或证明模块选择，更不能证明超时原因。
...
既有 mapping 执行器 23 根/67 verdict、进程拥有者 7 根/26 verdict 的可移植普通/race 检查通过；忽略 resume count、重复关闭 stdin 的两个隔离对照分别命中断言，程序/测试构建和 vet 已核对。当前 checker 的二十四项 Python 用例通过；恢复零输出条件与 Windows 原分隔符 key 的两个对照分别命中合法阶段输出拒绝和十 key 不匹配断言。可移植控制的 verdict 不包括 Windows 适配或 tagged 原生单元。[ece 原生运行 35439665455](https://github.com/codetreker/remote-fs/actions/runs/35439665455)的初始 inventory 在 4010 ms 超时，实际 resume count=1、写入 219 字节；109 字节 stderr 已观察到 entry→before-readline→after-readline→before-json，没有 after-json。输入 JSON/script hash 有效，四单元因原零输出准入条件而未启动；该拒绝保留原失败，不把最后 marker 定位为已确认的 JSON 或模块故障。四单元仍未原生执行，这些本地控制不验证映射、guest 或缓存，也不宣称修复四秒超时。
