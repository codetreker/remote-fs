...
[真实 HTTP/SSE 全负载可见性用例](../packages/storage/replicated/replica_acceptance_test.go)使用 4096 文件与 128 个持续回源 authority 列目录的读者，以一秒为独立写者提交后另一客户端的本地 Stat 观察到新元数据的上限。生产默认仍是 1 GiB body、64 个 active response、64 个 waiter 与 8 GiB aggregate；验收夹具在 client 和 server 两侧显式使用 2 MiB body、129 个 active response、64 个 waiter，以及 `4 * 129 * 2 MiB = 1032 MiB` aggregate，使 128 个 List 与另一个 authority write 或 Stat 都能进入 transport admission，避免 HTTP admission starvation 掩盖 SQLite 写者排序和 SSE 可见性。每个 reader 完成第一次完整 listing 后停在 barrier；全部 128 个到齐才同时释放持续负载，并在提交写入前确认 listing 再次推进。写入返回时记录起点与恰好一秒后的 deadline；该 deadline 前既要由另一客户端的本地 Stat 观察到新 metadata，也要有 authority listing 完成。fixture 的 90 秒 HTTP timeout 只防止 transport 永久挂起，不放宽这一秒判据。调用进入回调不构成负载成立的证据。
