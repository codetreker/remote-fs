...
修复脚本在 input/environment 观察后加入 before-utility、Core Import-Module 的验证绝对路径、after-utility，再执行原 before-json/JSON pipeline；成功导入输出被抑制，原 stdout 单结果保持。不带 Force、不做 Get-Command、不另起进程或重试，也不用 import-by-name fallback。可选 Utility facts 记录磁盘 path/size/read count/SHA256/耗时和清理，不证明实际加载依赖的字节。用例必须验证普通命令与 inventory 同一 builder、相同 deadline、没有过早准备/额外 stdout，以及导入失败保留原 ErrorRecord；SmbShare 与原 mapping 操作保持。

显式 Utility 修复的可移植 mapping 普通/race 各 38 根/139 verdict、tagged 准备/诊断各三根/3 verdict 通过；错误路径覆盖 missing/reparse/nonregular/超限、read/stat/close/cancel、无 launch、相同 deadline 与组合 cleanup。八项实际本地 PowerShell 7.5.3 AST 场景核对真实 JSON statement、导入输出抑制、原/次要错误与关闭 writer；Go 和 PowerShell 的导入遗漏对照分别命中断言。2040 字节/33 个 CRLF 是实际 formatter 分支配合合成最大事实的投影，不是自然出现的 mapping trace。三项当前 ARM64 程序/默认测试/tagged 测试构建、vet 与格式检查通过；既有 checker/control 证据仅对未变部分复用，修复后 Windows 原命令/nativefile/生命周期仍待实际运行。
