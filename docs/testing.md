...
[能力与 backend 控制](../.github/scripts/native-smb-posix-capability-controls_test.go.txt)核对精确 response 位、其它字段与 retained A/B 身份/字节；实际准备的非测试源码差异只有该位。适用本地控制共 41 根，普通/race 各 395 verdict 通过；恢复 server 位和仅改被动收到的位两个对照分别命中断言，十二项脚本策略用例及 POSIX/default/always 三种 ARM64 构建通过。default/always 原 server 输入与 native-call 序列保持原样。新单项尚未原生执行，既有 alias/retained-identity 失败不改写，不能把实际 backend 能力或本地控制当作重定向器身份修复。
