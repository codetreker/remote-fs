...
[真实 HTTP/SSE 全负载可见性用例](../packages/storage/replicated/replica_acceptance_test.go)使用 4096 文件与 128 个持续回源 authority 列目录的读者，以一秒为独立写者提交后另一客户端的本地 Stat 观察到新元数据的上限。生产默认仍是 1 GiB body、64 个 active response、64 个 waiter 与 8 GiB aggregate；验收夹具在 client 和 server 两侧显式使用 2 MiB body、129 个 active response、64 个 waiter，以及 `4 * 129 * 2 MiB = 1032 MiB` aggregate，使 128 个 List 与另一个 authority write 或 Stat 都能进入 transport admission，避免 HTTP admission starvation 掩盖 SQLite 写者排序和 SSE 可见性。128 个 reader 各完成第一次完整 listing 后，测试专用 backend wrapper 在 HTTP admission 之后截住各自下一次 `ListBounded`；独立 authority Write 也停在同一 backend 边界。两类调用全部到齐后只释放一次，使 128 个 List 与 1 个 Write 一起进入 SQLite 竞争。Write 返回时记录 `written`，以 `written + 1s` 为精确 deadline；此前既要由另一客户端的本地 Stat 观察到新 metadata，也要有至少一个 authority listing 完成。fixture 的 90 秒 HTTP timeout 只防止 transport 永久挂起，不放宽这一秒判据。调用进入 HTTP 或 backend wrapper 不构成负载成立的证据。
