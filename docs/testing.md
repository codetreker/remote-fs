...
[工作流](../.github/workflows/native-current-authority.yml)显式传 -CurrentSMBCold，独立调用默认仍为 HTTP-only。cold 只支持新鲜隔离的 GitHub-hosted Windows ARM64 job，并以本 fixture 为该 logon 唯一映射修改者；[driver/guard 用例](../.github/scripts/native-current-authority/run_windows_test.ps1)核对 runner 声明、run/attempt 和规范 artifact 路径，拒绝手工/自托管条件。该支持前提不是无外部 actor 的认证，也不跨任意 checkout root。顺序仍为 create/1 → cold/2 → authority stop/2、start/3 → reopen/2 → shutdown/4，同一不可变 seed 的 NodeID/epoch/source/nonce 贯穿；require-native 不被 cold 结果升级为完整 SMB/一秒验收。

[Windows host](../.github/scripts/native-current-authority/smb_probe_windows.go.txt)组合当前 SSPI/精确 SID、SMB endpoint 与 HTTP FileStorage，后端为同一 Linux guest 内的真实 authority。首次普通 CreateFile 后同一 HANDLE 核对 Basic/Standard/FileIdInfo、字节/EOF 与 opaque identity 重复相等，不要求非零或数值等于 NodeID。512 字节 allocation 是虚拟表示；一个对象不证明替换身份、写入、目录、断线或暖缓存。I/O/frame/HTTP body 仍为 64 KiB/128 KiB/1 MiB，8 MiB 现有 result pool 与 MaxOpens/MaxRequests16 保持，metadata 预留不借 frame 预算。

[被动观察器](../.github/scripts/native-current-authority/smb_observer.go.txt)保持 connection/session/tree/open、related allocation 及已支持流量的严格归属。实际当前 CREATE/READ、HTTP retained open、owned listener、认证与 signed traffic 仍须成立；QUERY_INFO/QFid 只核对真实已出现的请求/响应，metadata 来自 CREATE 时照实记录，不补发缺失查询。请求 integrity、拷贝到回执的 port、provider 成功或一个库存行都不能代替这些条件。

[一次性 helper](../.github/scripts/native-current-authority/probe.go.txt)在普通 probe 解析前分流；同一个 source-bound executable 经原 Windows Job owner 启动，非 Windows 明确拒绝。[协议与映射用例](../.github/scripts/native-current-authority/smb_mapping_windows_test.go.txt)核对唯一至多 4 KiB LF 请求/最终回复、run/source/nonce/volume/sequence、state identity、SID/LUID、drive/UNC/port、request digest 与实际 owner。helper 不等待 EOF、不另起可执行程序；重复/unknown field、错配/部分/超限帧、短写、无回复 exit0 不能证明 mutation 完成。原四秒涵盖 启动、全部 API 与解析，不在每页或子步骤重置；三 worker、stream cap、error chain 与 Job 清理仍归原 owner。

WNetAddConnection4W 只从 system MPR 解析，精确七参数、DISK NETRESOURCEW、NULL auth/length0、CONNECT_REQUIRE_INTEGRITY-only 及唯一 SDK transport option 分别验证。64-bit NETRESOURCEW 为 48 字节/alignment8；24 字节 option 的 header8、Wsk、TCP port/flag 与所有保留/padding/QUIC/RDMA/skip 零值按 host ABI 核对，UTF-16/pointer 在同步调用返回前存活。删除/改变 port 或加入禁用 flags 的对照必须命中断言。首次 source-bound hosted ARM64 运行已验证该 WNet4/provider 配对可用，精确 API 参数由源码与执行 binary 绑定，未保存成功调用的 raw syscall/helper frame；不可用 export/不支持选项/认证/签名/路由失败保持失败，不换 API、PowerShell/CIM/CLI、445 或其它策略。

Snapshot 使用 WNetOpenEnum/EnumResource/CloseEnum connected DISK、26 个 QueryDosDevice/GetLogicalDrives 和 26 个 WNetGetConnection 观测。固定 1 MiB 枚举 buffer、最多 128 总 entry/129 次调用、1025 UTF-16 lookup buffer 与解码后 1024-byte field 上限分别验证；字节与字符容量不混用。只在 NO_ERROR 验证 pointer/array/count/alignment/terminator 并复制有界 row，NO_MORE_ITEMS 只终止、不解析其未定义 buffer；MORE_DATA、额外 entry、无进展成功、坏指针或 CloseEnum 错误均失败。NOT_CONNECTED 与 CONNECTION_UNAVAIL 分开，后者不能当空闲；local row 与精确 lookup 不一致也不能通过。完整空集合不能从任何失败推导。

setup 用 WNet4 正常成功、connected membership、成功精确 lookup 和 DOS binding 取代旧 CIM Status==OK，不声明 health 等价或数据可达。成功创建后的独立观测失败必须和未取得完整 API 结果的 unknown create 分开；signed owned-port/native file 与全部生命周期 oracle 不变。probe 自身 24 秒、controller watchdog 30 秒及恢复另有 30 秒总期限保持，杀死 helper 不能宣称 provider 已取消或回滚。

固定 executable/root/state 路径与 protected owner claim 在首次库存读取前准入。按 SID/AuthenticationId 派生的 owner 目录独占创建、claim 同步且不可删除/重置；已有目录包括缺失、部分或坏 claim 均阻止同 root/logon 新 cold，成功后也不复用。用例覆盖新 run 子目录绕过、错误路径/owner/Job、非 Windows helper、unsupported-host 拒绝；只允许精确同次 cleanup 重入。driver 的部署声明不冒充对其它 checkout 或外部 actor 的全局锁。

create/remove 各自 durable pending 必须先于唯一 dispatch；完整绑定回复的 not-invoked/API-return 事实在 owner lock 内持久化，nil 只表示完整成功，私有 result-bearing error 保留原 error chain 与独立过程失败。terminal replacement 可以已发布字节再返回错误，读回 success 不能代替原调用方的 Save ACK。任何 mutable-ledger write/sync/close/replace 错误（含 observational Save）都不可逆撤销原 live 删除权限，同进程也不能从内存或读回结果恢复。丢失/畸形 reply、非零/ambiguous status、未确认进程清理或损坏/缺失 ledger 均不能被 exit0、空库存或 later-row absence 冲淡。

unknown create/remove sticky quarantine 下不再发映射修改，read-only 证据与本地 server/process/disk 清理仍可进行，随后丢弃隔离 runner。受控用例覆盖 timeout→helper 已停→库存为空仍为 unknown、迟到回执、先看到行再消失、remove 丢回执、create-success frame 但 Job 未静止、保存失败和readonly 初始失败仍消耗 claim。没有 replay、force removal 或第二API尝试。

仅原始 live attempt 的不可序列化 removal authority 可授权一次正常删除，授予条件是 validated create API NO_ERROR、create-result Save 的调用方成功 ACK、helper Job 静止与 streams 收齐。纯 snapshot 读取失败可以保留权限，但修改前必须新读完整匹配 ledger，并通过 owner/baseline/精确 drive-remote-DOS 核对。权限在尝试 remove-pending 保存或 dispatch 前消耗，失败也不能复用；WNetCancelConnection2W 仍为 local drive/flags0/forceFALSE，并要求正常 API 返回、完整后置缺席与 baseline 不变。

cleanup-only 是失败路径恢复，正常 verified cold success 绕过它。重入对映射始终只读，成功 terminal 字节或当前缺席不能补证 Save ACK、已结算 removal 或重建权限，未证明的恢复状态仍须返回错误/保持隔离。用例须覆盖实际 publish-then-error 保存、observational Save 错误、恢复进程读到 success、同进程读回以及 remove-pending 出错后再调用，全部不能获得第二次修改；同时保留 read-only 观测失败后以原 live 权限重新核对的区别。原始进程退出丢失权限，不增加确认 journal/marker/manager。公共取消接口没有 generation compare，fresh 唯一 actor 的前提保持显式；本地清理与只读证据不能消除原 mutation/publication 不确定性。

PowerShell mapping、旧诊断tag和自动后置六单元已退役；普通测试清单、source/Job/native-unit/cold 门禁保持。[旧对象绑定版本首次运行 35459298859](https://github.com/codetreker/remote-fs/actions/runs/35459298859)在 4010 ms 初始库存失败，实际 System32 ARM64、resume1、219 字节写入与当前 script已记录，但 stdout/stderr 为零且未见 entry，不能证明进入 import/object/nativefile。后置库存 3793/947 ms 通过、discovery 4020 ms 失败、import/parse 480 ms 通过，不证明原失败来自 binding/parser。更早 PowerShell 结果见[拥有说明](../.agents/notes/implemented/testing/2026-09-19-current-authority-virtual-machine-fixture.md)，属于各自 source 的历史证据，不指导当前执行。

可注入 mapping 控制普通/race 各 25 根/141 verdict 通过；省略 TCP port、省略 integrity、旧 inventory key、重建 cleanup authority 和丢弃 command error 的五项因果对照命中断言。真实 ARM64 probe 程序与完整 probe-unit binary 已构建并通过 vet/格式检查；其中 Windows 实际磁盘 publish-then-EIO、private claim/file-lock 两根在本地仅编译，随后在首次 WNet 原生运行中实际通过。支持层的 Linux probe 普通/race 各 35 根/215 verdict、本地 PowerShell 7.5.3 Unix guard 37 项和 Python checker 7 项通过；它们不证明 Windows junction/DACL/锁或 WNet 调用。

Windows helper 单元不得在原 cold 前建立映射或读取 sentinel。[首次 WNet 原生运行 35471060655](https://github.com/codetreker/remote-fs/actions/runs/35471060655)绑定 `8b403735f2e87a00babbddc0c2bb2383ad40dfd0`，在 fresh hosted ARM64 上通过单对象 cold 与 authority/VM 生命周期：create/remove 返回零且 helper 静止，owned port 62811 的 52 个 post-auth signed packets 与 lineage、HTTP/SMB/native 的 173 字节、EOF 和同一 HANDLE 重复 FileIdInfo 匹配；原 live attempt 完成移除，普通重启后以新 epoch 重开同一对象，QEMU 自然 exit0、非 forced Job 及磁盘/基底清理通过。native probe 69 根/379 verdict、controller 62 根/254 verdict 与 40 项 Windows 路径检查通过。signed flags/lineage 是实际观察，签名加密验证仍由源码绑定，未独立重放。该验收限于本次 fresh runner 的 provider/options 与单对象路径；`native_acceptance` 保持 `not-run`，不能据此推导暖缓存的一秒可见性、replacement identity 或跨对象 ID 唯一性。
