...
[文件句柄设计](design/server/file-handles.md)按对象身份、逐次发布、identity namespace、目录／名字观察、action receipt、durable deletion、metadata、Use claim、范围与会话生存期分别验证。[公开类型用例](../packages/storage/files_test.go)、[路径用例](../packages/storage/storage_test.go)、[metadata 用例](../packages/storage/metadata_test.go)、[DirectoryReader facet 编译期断言](../packages/storage/directory_reader_test.go)、[目录 metadata 值类型用例](../packages/storage/directory_metadata_test.go)、[目录观察值类型用例](../packages/storage/directory_observation_test.go)、[名字观察值类型用例](../packages/storage/name_observation_test.go)、[能力校验用例](../packages/storage/capability_validation_test.go)和[range 用例](../packages/storage/ranges_test.go)拒绝无访问权限、非法打开效果、畸形父身份／叶名、任一规范化路径 component 超过 4096 字节、缺失或复用错误的 action、无界 metadata predicates、矛盾或超限 guards、错误 pending generation、非法 Use/range 与 action epoch/nonce。ChildSelection 必须先 Check 原始 slice，再分配 clone；clone 独立拥有叶名、Scope、directory revision 与 edge leaf，nil guards 保持缺席。空 metadata token 必须稳定表示 namespace 缺席。NamespaceAccess、DirectoryReader 与 DirectoryMetadataObserver 在 Go 类型和 wrapper preflight 中分别核对；实现缺少某项能力时明确拒绝，不能重新按路径打开或用本地锁模拟。
...
[NodeReference 与原子打开用例](../packages/metastore/sqlite/identity_capabilities_test.go)覆盖普通文件、目录及符号链接引用，父目录改名与旧名字复用，Keep/Reset/Replace、并发 create winner、metadata predicates、Use claim、close intent 与原 action Attr/Outcome。[guarded child selection 用例](../packages/metastore/sqlite/guarded_open_test.go)证明内容修改不使目录 guard 失效；目录 revision 或 edge 变化使 OpenAt/OpenChildRef 在创建、清空、替换、引用、Use 与 close intent 之前以 `ErrConditionConflict` 失败，且不留下任何部分效果。它还证明 preflight 只验证父 DirectoryTarget／Scope；父／Scope、guards 和 child facts 在同一个最终 authority transaction 中重新核对，之后才推导 publication intent，并区分不发布的 Keep 打开与实际 mutation。返回 error 但带引用时仍须完成清理；去掉 Scope/State 转发、把 guard 检查移出最终 authority transaction 或把条件拆成先查后改，应使契约或 wrapper 用例失败。

[identity namespace 用例](../packages/metastore/sqlite/identity_capabilities_test.go)在 retained parent rename/reuse 后执行 LookupAt、create、remove 与 rename，分别核对源、被替换目标和 output slot。metadata 条件与名字效果必须在同一事务；条件失败、第三对象占位或 scope 失效均无部分修改。[FUSE capability 用例](../packages/fuse/capabilities_test.go)要求 NamespaceAccess 与 DirectoryReader 两个独立 facet；[directory 用例](../packages/fuse/directory_test.go)与[namespace 用例](../packages/fuse/namespace_test.go)验证 Opendir/Readlink 的 NodeReference、普通子项操作的父 NodeID、directory-handle Lookup 与 Readdir 的额外 Scope、符号链接创建/读取，以及打开目录改名并复用旧路径后枚举仍作用于原对象。

### 名字与目录观察

[SQLite 名字观察用例](../packages/metastore/sqlite/name_observation_test.go)核对 Root、Linked、rename 后的新绑定、unlink 后的 Detached、stale directory guard 不暴露部分 entries，以及收紧后的 directory entry／byte limit。exact scoped ReadDirNode 在目录 unlink 后继续返回原 detached 空目录；bare NodeID、错误 Scope 与 DirectoryMetadataObserver 都拒绝该目标。普通 List、bounded List、ReadDirNode 和 directory metadata 遇到 dangling child 或仍具名却标为 detached 的 child 时整份以 `EIO` 失败，不遗漏坏条目后返回其余成员。它还验证并发 create 和每类成功名字 mutation 都使受影响目录的 opaque revision 单调改变，失败与 no-op 保持 token，跨目录 rename 推进两端，revision 耗尽使 mutation 整体回滚，并在重开后核对 token 与完整 entries 保持。

[objectstore 目录观察用例](../packages/storage/objectstore/directory_observation_test.go)核对独立权限面、实际 prefix 预算、guards、Scope、取消、身份替换和畸形 backend；[reference name 用例](../packages/storage/objectstore/reference_name_test.go)在 rename、旧名字复用与 unlink 后持续核对原 NodeID，拒绝 stale guard，并证明名字观察不读取内容。[SQLite replica revision 用例](../packages/metastore/sqlite/replica_test.go)分别核对原生 snapshot/change 的 opaque token 保真，以及 revisionless v5 输入生成持久本地 token、namespace replay 使它改变、reopen 后保持。[HTTP replication 用例](../packages/transport/httprest/replication_test.go)断言 v5 Node 编码省略 DirectoryRevision、既有无字段形状仍可解码、携带该扩展字段的输入被严格拒绝。revision 是不透明等值 token；测试不能用增量大小推导一次高层操作发生了多少内部名字集合变化，也不能把 replica 本地 token 当成 authority 证据。

结果预算分别卡在 65,536 entries、8 MiB native retention、名字观察 prefix、4096-byte leaf、64-byte revision、每类 256 项 guards 和 64 KiB guard retention 的精确边界。SQLite 另以更小的 `MaxDirectoryEntries`／`MaxDirectoryBytes` 验证实际 serving limit，负值或硬上限以上配置在接触数据库前拒绝。生产方必须在载入叶名或 metadata 前 reserve，任何单项超限、中途错误、取消、错误 identity 或 wrapper 验证失败都使整个 ListResult 与 observation 不可读取；不能留下成功前缀。limited、locked、objectstore、localstore、replicated 与 HTTP 用例分别验证能力 preflight、读取 context、session/reference drain、ChildSelection 深拷贝与完整转发，以及副本失效后不回源猜测；replica 的本地 directory revision 不能被当成 authority guard。

HTTP 用例把 `file.read-dir-node`、`file.observe-directory-metadata` 与 `file.observe-name` 分别授权，拒绝时 registry 和 native backend 零触碰。strict DTO 覆盖 lower-camel 字段、canonical base64、缺席／null／重复／未知字段、非法状态组合、NodeID 替换、目录自身名字选择与 ResultBytes；server、client 或 native 任一预算更紧时整次失败。OpenAt 与 OpenChildRef 保留顶层 `child` 并可携带顶层 `guards`，server 组装 ChildSelection；畸形或超限 guards 在授权和 backend 前拒绝。NameCommand、FileMutation 与显式 delete-intent 请求仍不得携带 guards。

### 节点事实与 metadata

[metadata 编码用例](../packages/storage/metadata_test.go)覆盖 namespace 字符集、数量、单值和总 envelope 边界，要求 canonical key 顺序、非空 authority version、present-empty payload、深拷贝与畸形输入拒绝。[SQLite metadata 用例](../packages/metastore/sqlite/metadata_access_test.go)通过 NodeID 和 retained File 分别执行缺席条件、版本匹配、冲突、不相干 namespace 保留、ChangeTime 推进与 mutation barrier；条件失败不得改变 payload、版本、时间或日志。
...
schema v9 用例从真实 v8 记录前滚，核对原 intent ID 作为 owner、每 volume 的序号分配与高水位；迁移后用原 ID 继续查询。损坏的 owner、sequence、唯一性或回退的高水位在 serving 前失败。新接受在同一事务分配下一序号，ACK 不回退高水位，耗尽 `MaxInt64` 明确拒绝。
...
Use/range 生命周期用例还要区分持久数据与内存控制状态：SQLite 重开后 NodeKind、共同时间、opaque metadata、directory revision、pending state 以及 delete intent 的 owner、序号与高水位保持，旧 authority 的 File、NodeReference、scope、UseOwner、range 与有限 FileAction history 全部失效。新会话可以按 owner 发现、按 ID 查询 durable intent 并重新取得普通状态，但不能恢复或静默重绑旧持有者；Strong S/X 的持久恢复用例保持独立。
...
[FUSE Session 用例](../packages/fuse/session_test.go)让多个成功续租跨过原始期限后继续读取同一引用，再显式停止 Session，核对旧引用已失效、调用方拥有的 storage 仍可用。续租失败不能延长最后确认期限。连续性丢失后等待后台退出，再核对整个挂载的健康检查与停止结果持续为 `EIO`，原 FileSession 的引用不能继续写入；停止会取消在途 Renew、等待 Session 退役，并让重复停止共享同一个关闭结果。挂载级终止后需要新挂载建立新 Session，native Range Drop 的 Owner 级清理继续独立成立。迟到的 Status 回复不能从收到回复的时刻重新起算租期。

native admission 用例用一个暂停的 Put 占满 Session 唯一的 data slot，先确认普通 File.Stat 为 `EAGAIN`，再核对 Renew 的 revision 前进且 Status 仍可用。另一用例暂停最终发布，验证静态能力探测可以完成，等待 contextual authority 的新 Session 创建可以取消。四类 admission 分别验证 data 的 MaxOperations，以及心跳、range 获取、核对与释放三个各 2 个活跃调用的分区。占满获取或核对分区后，同类调用为 `EAGAIN`，Renew 与数据访问仍能完成；获取饱和时 Range Drop 仍可释放。心跳名额也单独验证满额拒绝与取消归还，Close 在控制调用仍等待时继续前进。续租或 enrollment 不能因为大文件 staging 而变成不可退出的全局等待。

[HTTP 句柄用例](../packages/transport/httprest/file_test.go)用真实 SQLite/objectstore 验证同一对象的覆盖、unlink、替换与跨 handler advisory 协调。丢失 Open、Truncate、ACK 或 Close 回复后核对原动作，不能制造第二个引用或重新执行一次修改。[identity capability HTTP 用例](../packages/transport/httprest/file_identity_capability_test.go)另行证明已完成 guarded open 的响应丢失后，即使 namespace 在重投前改变，也返回保存的原结果而不重新核对 guard。Handler.Close 只退役自身登记的 Session，另一 handler 的会话仍可用。[名字观察 HTTP 用例](../packages/transport/httprest/file_name_observation_test.go)覆盖完整往返、预算、strict DTO、身份复核和旧 Open 回复丢失后的 reference identity。[registry 用例](../packages/transport/httprest/file_internal_test.go)覆盖 Session/action 上限、未 ACK 的 Open 在 Session 持续 Renew 时仍到期回收，以及旧窗口或已逐出的动作不能再次执行。
...
[副本句柄用例](../packages/storage/replicated/files_test.go)让 retained File 的读取、属性与锁控制直接核对 authority。带名字的创建和修改确认 metadata barrier；unlink 后继续操作旧对象不制造新名字或复制事件，原 Position 可以保持不变，同名替代物不受影响。[副本名字观察用例](../packages/storage/replicated/name_observation_test.go)核对目录／名字观察先要求副本和 session 健康，再回源取得权威结果，并保留 prefix budget、identity、context 与零失败结果。[失败用例](../packages/storage/replicated/file_fault_test.go)与[admission 用例](../packages/storage/replicated/files_internal_test.go)覆盖缺失、错误或负 barrier，未返回 Open 的引用回收，已发出修改的原动作核对，以及共享 confirmation pool。MaxFileSessions 同时计入正在远端创建、仍存活及 cleanup 未确认的会话；发送前满额为 `EAGAIN`，确认 Close 才归还名额，未知清理继续占用。wrapper 不替 FUSE 启动续租 timer。

### 内核入口验收

[live file 用例](../packages/fuse/live_files_test.go)使用两个各自持有 HTTP client、metadata replica、FileSession 与 inode cache 的真实挂载点，并使用交付的 direct I/O 配置。读描述符先读过旧范围，再由另一挂载点覆盖、增长、缩短至零，逐次核对相同 inode 的内容、大小和 EOF；rename、unlink 与替换后对旧描述符写入和设置属性，原对象与名字上的替代物分别读取。WriteAt、普通写和未写字节的 O_TRUNC 都须在 Close 前从 authority 可见；相邻范围与重叠修改按实际执行次序保留结果。并发 Create/Open 验证排他创建只有一个成功者，非排他调用打开同一赢家并保持既有 POSIX metadata。
...
SQLite 只读用例覆盖查询取消、预算回调取消、回调成功后才取消、`database/sql` 自动回滚产生的直接 `ErrTxDone`，以及真实查询和 cleanup 故障。[连接归还回归](../packages/metastore/sqlite/transaction_lifecycle_test.go)用真实 modernc transaction 在 driver Rollback 中设置门，分别触发取消和 deadline；回滚仍持有连接时，读取不得返回。释放门后验证原原因与 `EINTR`/`EIO` 分类、pool InUse=0，并立即正常关闭 store。修复用例以本次绑定 Conn.Close 的进入信号确认开始等待，释放 driver 门后才取得读取结果；不能用轮询池或 Close 重试消除窗口。

同一组真实 modernc driver 门还通过 `Store.Create`、`Replica.Apply` 和 Replica Seeding 的公开生命周期入口覆盖取消中的 mutation：修复用例在回滚释放前阻止操作结束并保持 commit/replica gate 与原生所有权；释放后核对 writer InUse=0、正常 Close、原生所有权重新获取及修改未生效。成功的 `Seeding.Complete` 另在绑定 connection close 处暂停，验证完成结果与两个 gate 都不能提前发布。读取 setup failure、snapshot 结束、普通读取及已 Commit 事务覆盖绑定连接的单次清理，重复收尾保留首次 Close 错误；独立 Close 错误（含 context 原因和 `ErrConnDone`）须保留 `EIO`，不能借直接 `ErrTxDone` 的取消规则吞掉。[启动准备清理用例](../packages/metastore/sqlite/internal/schema/prepare_cleanup_test.go)通过真实 `PrepareConfigured` 入口核对自动回滚期间不提前返回、BeginTx 失败仍关闭连接，并直接验证已知/未知 commit 与独立 cleanup 错误分别保留。[自动回滚 snapshot 用例](../packages/metastore/sqlite/snapshot_test.go)用确定性调度等到回滚静止后核对原 context 原因及重复关闭，不以池计数轮询同步。SQLite code 9 只有与实际已取消的读取 context 同时出现时才按取消解释。每种情况都验证原因保存和最终 errno；未知 commit 与 poison 仍为 `EIO`。
...
FileAction 用例对 OpenAt/OpenNodeRef/OpenChildRef、MutateName、conditional mutation、pending set/clear 与 delete-intent ACK 注入回复丢失和取消：同 ID、同语义输入只执行一次并返回原结果，不同输入拒绝；objectstore 与 HTTP 对 ChildSelection 使用同一集中式 canonicalization，只重排 guards 或在 nil／present-empty 之间变化仍重放原结果，改变 guard 事实、ChildName 或目标条件则以 `EINVAL` 拒绝。查询保留记录带原 Operation，缺失记录只能返回空 Operation 的 not-executed/unknown/retired。completed 或 pending 不得缺失 Operation。authority epoch 前进后旧动作不能重新执行，只有明确 not-executed 允许调用方创建新的逻辑动作。

[HTTP 关闭恢复用例](../packages/transport/httprest/file_close_recovery_test.go)让同一次 Close 同时发生引用释放和删除语义错误，断言 `CloseWithResult.Released` 如实报告责任；未确认释放保持 false 与错误，成功保持 true，取消不能自行推出释放事实。`file.close` 与 `file.session-close` 以 `FileActionID` 重投，在能力退役后仍能查询原生动作状态，再通过原动作重投取回 `closeResult.released` 和语义错误，同一关闭不重复释放。另在原生关闭已释放引用后使 mutation barrier 查询失败，核对无 barrier 的错误响应同时携带 `released=true` 与 `barrierPending=true`；client 返回保留 cause 的 `CloseBarrierPendingError`，`QueryFileAction` 的 Completed 不证明 barrier 已知。重复 CloseWithResult 或 CloseWithBarrier 必须保留原 ActionID，直到重投清除 pending；重投的网络故障也不能撤销此前确认的 Released。确认后可以返回原生语义错误；无 Log 时 `barrierPending=false` 与 nil barrier 也是终态，不能误判为待确认。终态会话回执占用有界容量，达到 `2 × MaxSessions` 的合计上限时新会话以 `EAGAIN` 拒绝，历史到期后重新接纳。

[SQLite 删除义务 owner 用例](../packages/metastore/sqlite/delete_intent_owner_test.go)用两个 owner 建立多条义务，核对按 owner 的 list/query/ACK 隔离、游标单调推进、ACK 后空洞不重用、重启后继续分页及错误 owner 零效果。边界用例另核对 256 条页上限与 `MaxInt64` 高水位拒绝。ACK 丢失仍通过原 FileActionID 核对，已 ACK 的 ID 不被当作未发生的旧义务。

[打开 ACK 用例](../packages/transport/httprest/file_client_test.go)在真实 Open 返回引用后取消 ACK，覆盖按路径只读、读写以及按节点读写的普通已有文件打开；要求没有返回 File、原取消原因和规范 EINTR 保留、ACK 发出零次，同一 Session／File 的清理确认一次，native open／close 各一次，属性与字节不变，unlink 后无残留引用用量。创建、截断、报告 DeadlineExceeded 的 context、已发送但丢失且核对失败的 ACK、会话关闭，以及清理 EIO／ESTALE 仍为无 File 的 EIO；创建和截断效果如实保留，未成功清理的引用仍占用实际用量。该 deadline 用例验证 ACK 前的错误分类，既有真实计时器超时用例继续覆盖时间到期。清理必须原始返回 nil 且报告 Released=true，不能把 ESTALE 的后续抑制当作成功；既有丢失 ACK 后成功核对的用例保留。
...
[SQLite 分配用例](../packages/metastore/sqlite/allocation_test.go)与[迁移及损坏用例](../packages/metastore/sqlite/internal/schema/allocation_test.go)用 0、1、4096、4097 字节文件分别核对 `Attr.AllocationKnown`、`AllocationSize`、精确 `Usage()`、`Space.Used/Avail` 与 `EDQUOT`：已占 cluster 内的增长不收费，跨 cluster 增长一次收满 4096 字节；失败、结果未知或未完成的缩短不能提前释放额度。目录、符号链接及空文件的已知分配量为零，零不是未知的替身。删除名字后的活引用继续收费，最后引用回收和重启清理各只退款一次。旧 authority 数据库的 v10 迁移核对所有 volume 的当前节点、detached 节点、retained change 与分配总账；[replica 分配用例](../packages/metastore/sqlite/allocation_replica_test.go)核对旧 replica 行保持未知、已知的其它来源粒度保持原值、本地 `Space` 不报告 authority 用量。已超额的 authority volume 仍可读取并缩减、`Avail=0`；负数、溢出、未对齐、种类不符或篡改总账在服务前失败。[HTTP 属性编码](../packages/transport/httprest/message_test.go)中的 Attr 必须保留 `allocation_size` 与 `allocation_known`，[复制编码](../packages/transport/httprest/replication_test.go)中的快照与 change Node 必须保留 `allocation_size` 与 `allocation_known`；缺失或非法字段是协议故障；[HTTP 会话能力用例](../packages/transport/httprest/file_capabilities_test.go)核对 `AllocationReporting.CheckAllocationReporting()` 对完整包装链的保证，调用成功后仍验证返回值。[FUSE 能力预检](../packages/fuse/capabilities_test.go)核对未知来源在挂载前失败；[FUSE 属性用例](../packages/fuse/metadata_test.go)核对块数由已知分配量投影，未知值明确失败。[limited 分配遮蔽用例](../packages/storage/limited/files_test.go)核对精确 payload 配额包装把全部 Attr 分配事实标为未知，且会话能力检查返回 `EOPNOTSUPP`；FUSE／SMB 用各自的效果前预检覆盖该拒绝。
...
[server 用例](../packages/transport/httprest/lock_server_test.go)验证 `/v5/` 协议标记和旧版本拒绝，以及匿名与 scoped 调用都抵达同一个执行保护的 volume。Scope header 的空值、重复值、错误 base64url、缺失或重复成员、未知字段、超长值和错误使用位置必须在修改前拒绝，随后 Stat 证实目标未创建；读取与控制操作不接受 mutation scope。能力值只在 body/header 中传递，URL 与错误诊断不能泄漏它们。[Scope 用例](../packages/transport/httprest/lock_scope_test.go)逐一验证所有 mutation、WithBarrier 与无效果 mutation 保留复制后的 proof，读取不发送它，匿名 handler 不继承被包装客户端的权限。

[文件能力 HTTP 用例](../packages/transport/httprest/file_capabilities_test.go)验证 server 按实际包装链宣告 AtomicOpen/Namespace/References/FileActions/Metadata/Owners/Ranges/DirectoryMetadata 与引用的 Scope/State/Delete/Conditional/ReferenceName。OpenAt/OpenChildRef 保持 `child` variant，并接受可选顶层 `guards`，OpenNodeRef 保持 node variant；三者的 action、node/outcome、partial reference、ACK 与 barrier，LookupAt/MutateName、条件 mutation、pending set/clear、QueryFileAction、按 `deleteOwner` 分别 list/query 的义务及携带 owner 的 ACK 都按各自封闭形状核对。`file.close` 与 `file.session-close` 的 action、退役后回执、`closeResult.released` 与 `closeResult.barrierPending` 在成功、语义错误和 barrier 未确认响应中分别核对。空 Operation 只允许没有 retained record 的 not-executed/unknown/retired receipt。[名字观察 HTTP 用例](../packages/transport/httprest/file_name_observation_test.go)覆盖 ReadDirNode/ObserveDirectoryMetadata/ObserveName 的严格 variant、canonical base64、ResultBytes 和身份核对，并证明 DirectoryReader-only 或 DirectoryMetadataObserver-only session 都不会宣告 DirectoryMetadata bundle，只有两个 facet 同时可用才宣告；Namespace bit 与该 bundle 独立，remote client 也拒绝只有旧 Namespace bit 的 v5 directory read。metadata request 的空 token、字段上限和清理 ownership 分别核对；未携带 guards 的既有 child 请求保持原形，携带未知 guards 的旧 peer 明确失败，未知字段继续失败。
...
[真实 HTTP/SSE 全负载可见性用例](../packages/storage/replicated/replica_acceptance_test.go)使用 4096 文件与 128 个持续回源 authority 列目录的读者，以一秒为独立写者提交后另一客户端的本地 Stat 观察到新元数据的上限。生产默认仍是 1 GiB body、64 个 active response、64 个 waiter 与 8 GiB aggregate；验收夹具在 client 和 server 两侧显式使用 2 MiB body、129 个 active response、64 个 waiter，以及 `4 * 129 * 2 MiB = 1032 MiB` aggregate，使 128 个 List 与另一个 authority write 或 Stat 都能进入 transport admission，避免 HTTP admission starvation 掩盖 SQLite 写者排序和 SSE 可见性。每个 reader 的首次 `ListBounded` 就属于受测负载：测试专用 backend wrapper 在 HTTP admission 之后截住全部 128 次调用，独立 authority Write 也停在同一 backend 边界。两类调用全部到齐后只释放一次，使 128 个 List 与 1 个 Write 一起进入 SQLite 竞争；测试结束前要求 128 次首次 listing 全部成功。Write 返回时记录 `written`，以 `written + 1s` 为精确 deadline；此前既要由另一客户端的本地 Stat 观察到新 metadata，也要有至少一个 authority listing 完成。fixture 的 90 秒 HTTP timeout 只防止 transport 永久挂起，不放宽这一秒判据。调用进入 HTTP 或 backend wrapper 不构成负载成立的证据。
...
session lifecycle 用例专门覆盖四条不能由普通成功 transcript 推出的顺序：未完成 secondary authentication 到期后自主回收且不损伤已建立身份；同 SID 不同登录会话的 reauthentication 被拒绝；identity expiry 使普通命令得到 signed session-expired，而同一身份 reauthentication 与 LOGOFF 保持可用；并发 TREE_CONNECT 在 LOGOFF 发布 retirement 后不能安装迟到的 authority/tree 或泄漏 global session charge。session retirement 还必须把最后一份 response frame 纳入 signer ownership，完成签名之前不能销毁 key，完成后必须释放。认证 provider 的 Close 失败、FileSession Close 失败与续期失败均保留 owner 和容量，cleanup 使用新 context 重试。Unpublish 用例把另一 export 的 FileSession.Close 阻塞住，证明未使用的 export 不加入无关 connection cleanup；目标 orphan authority 与无关 session 的进行中 SESSION_SETUP 并存时，目标 cleanup 不等待该 authentication。已经退休的目标 owner 若仍在 authentication 中，retirement bookkeeping 的等待必须在调用方 deadline 返回；authentication 完成后重试释放 export 与 global session capacity。另一条用例让 disconnected retired session 的 native authority close 先完成，再在 retirement commit 前取消；第一次调用必须保留已关闭 authority、export、signer、session charge 与 connection，重试随后释放全部 owner 并恢复 global capacity。两 export 用例还要证明先关闭的目标只释放自己，未关闭的 authority 继续保有 session retirement，最终 export cleanup 才释放 signer、session 与 connection。对确实共享 owner 的 cleanup lock 等待同样服从 deadline 并可重试完成。
...
当前 schema 与历史迁移都要用 corruption fixtures 验证每个 volume 的可见节点恰是一棵 rooted tree：root 无 incoming entry，其余可见节点恰有一个同 volume parent，并且全部可达；cycle、未标记的孤儿与跨 volume entry 都失败。detached retained node 必须是非 root、没有 incoming entry，且不作为任何 entry 的 parent/child；detached 目录保持为空。每个目录必须带合法非零 revision，非目录必须为空；retained change 的 kind/revision 组合保持一致。另用整数、负数、溢出与 mismatch fixtures 验证 `volumes.used` 等于全部 regular-file size 的 streaming sum，`volumes.allocated_used` 等于当前与 detached regular-file 的 4096 字节分配量之和；节点与 retained change 的分配量还须符合 kind、size 与对齐规则。`metadata_used` 等于节点与 retained history 的规范 metadata、link target 和 directory revision 长度。storage-class fixtures 把 entry/change name 改成 TEXT、把 schema/database/binding/log scalar 或 nullable change group 改成错误的 NULL/type，并构造非法 NodeKind、directory revision、position、metadata、size 与 nanoseconds；snapshot/log 不能漏行或接受 driver coercion 产生的可信零值。日志 fixtures 删除中间或尾部 retained change，并分别篡改 `previous_position`、`trimmed_through`、`committed_position`：`Open`、`Snapshot` 与 `ObjectStatus` 的完整链验证必须以 `EIO` 失败且不写入新 incarnation；`Since` 允许缺口之前的完整 page，跨到缺口的 page 必须整体失败且不暴露该页 prefix。空或较小日志在 caller 给出很大 limit 时仍按实际 anchor/row work 成功，page budget 耗尽且 tail 尚未返回时才以 `EFBIG` 拒绝。整链 record ceiling 由 `Open` 与 `ObjectStatus` 的精确边界／超限用例直接覆盖；snapshot row production 另有 caller-owned byte-budget 故障用例。`MaxIntegrityBytes` 与 `MaxMetadataBytes` 用各自精确边界验证 `EFBIG`；legacy migration 直接覆盖 record ceiling。ID fixtures 删除 node/change sequence、协同回退 internal high-water/sequence、把高水位压到其它 volume 的引用之下，并覆盖 node/change exhaustion；另抬高 committed tail，断言 append 在发布新 witness 前回滚。本地 durable fixture 再用外部 accepted state 拒绝协同回退。每一种拒绝都要断言版本、schema 与数据没有部分前进；volume 结构在 `Open` 与 `ObjectStatus` 两条入口覆盖，database-wide sequence/witness 拒绝由 durable open、page anchor 和分配路径覆盖。
