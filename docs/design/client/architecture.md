...
内核的目录项超时、属性超时、负项超时都是 0。查名字、问属性、列目录仍落到 storage，由本地 SQLite 查询答复。挂载呈现层不向内核发送失效通知；副本转入不可用时，到达这一层的操作直接返回 EIO。

这三个超时不关闭文件页缓存。`Open` 使用默认缓存标志，同一 inode 的页可以被不同 handle 共用，旧 handle 填入的内容也可能被新 open 读到；[跨句柄页缓存陈旧](../../../.agents/notes/proposed/bug-fix/2026-09-07-prevent-cross-handle-page-cache-staleness.md)记录这项缺陷。内核直接满足的读请求不经过 storage 的可用性判断。

副本以变更流的连续观察状态决定是否作答：流被观测为断开时作废，每一个到达 replicated storage 的操作以 EIO 失败（R-ERR-1、R-ERR-2）。没有过期时间或回源校验。续订在追到 opening tail 后恢复可用；快照重建却在灌完快照后立即恢复可用，积压仍未追上，详见[重建副本等待重放](../../../.agents/notes/proposed/bug-fix/2026-09-07-gate-rebuilt-replicas-on-replay.md)。
...
到达挂载呈现层的 storage 错误不会被改写为空目录或不存在；目录大小、扩展属性等无法回答的操作同样返回错误。页缓存与副本在第二节列出的缺陷仍能绕过这条错误路径并返回陈旧答案，违反 R-ERR-1、R-ERR-2。分类与阶段判定的取舍见[请求中断](../../../.agents/notes/implemented/bug-fix/2026-08-22-eio-from-a-freshly-mounted-mountpoint.md)。
...
- **不按 TTL 刷新命名空间。** 本地副本由变更流维护，作答条件与缺口见第二节；remote storage 不跨请求保留答案。挂载层持有打开文件的内容和剩余空间数字，内核还持有文件页缓存。
- **不复制文件内容。** 元数据副本只有树：名字、类型、属性、大小。打开文件时向服务端取回内容，后续读取使用 handle 缓冲区或内核页缓存。
- **不重试失败的写入。**
- **不在离线时接受写入。**
- **不代答命名空间答不出的问题。**
