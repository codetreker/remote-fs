...
中立节点事实与 metadata 位于根 package 的原发布顺序中。`namespace.go` 与 `namespace_identity.go` 分别支撑 NamespaceAccess 和 DirectoryReader：前者处理 LookupAt／MutateName，后者允许 exact scoped read 继续枚举 detached 空目录，同时拒绝裸身份和损坏 child binding；`directory_metadata.go` 只观察具名目录。`namespace_mutations.go` 原子执行名字效果；`namespace_revision.go` 分配并推进目录名字集合 revision；目录 metadata 与 reference name 查询在同一 read transaction 核对 scope、guards、entries、revision 和绑定。`atomic_open.go` 与 `node_references.go` 的 preflight 只验证父 DirectoryTarget／Scope；各自最终 authority transaction 再次验证父／Scope，通过同一 guard checker 核对 guards，随后选择子项、核对条件并推导动态 publication intent。只有实际 mutation 才发布，Keep 打开已有对象不制造发布。初始状态、Use、带 owner 与序号的 delete intent 和引用保留仍在同一最终顺序中组合；`node_references.go` 返回无字节方法的保留节点。`conditional_file.go` 在最终发布处比较 size/metadata 条件；`pending_unlink.go` 持有 durable intent、按 owner 的发现与查询、generation 和恢复清理。`metadata.go`、`file_access.go`、`reference_scope.go` 与 `reference_order.go` 继续把 metadata、Use、scope 和最终访问绑定同一个 native gate。它们复用 databaseCoordinator、retainedFile 和 change log，不建立第二套事务所有者。
