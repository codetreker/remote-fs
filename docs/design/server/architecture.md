...
server 为这几个操作持有的资源都有上限：同时开着的快照数、一份快照最长可以送多久、一帧多少行、一次读日志最多多少条（R-INT-3）。快照占着 storage 里的一个资源 —— 在 SQLite 上是一个读事务 —— 而它活多久由网络决定，所以已经开满时新的请求以 `EAGAIN` 拒绝而不是排队：client 是先订阅再来要快照的，重试对它不花什么代价。
...
收到 SIGINT／SIGTERM 后它停止接受新连接，并给在途请求 5 秒走完 —— 一次被切断的写入会让调用方无从判断它究竟发生了没有。收到 SIGHUP 则重新走一遍命名空间并替换计数，这是计数漂移之后回到实测值的唯一途径（见第七节）；重新测量期间每一个请求都在它本来就要等的地方等着，结束后重新量到的已用量与它之前的计数一起写到标准错误。没有配额的 server 收到 SIGHUP 时说明自己没有可重新测量的东西，然后继续服务。
