...
| **保留节点与中立访问** | FileSession 拥有 File/NodeReference、身份目录枚举、名字观察、Use claim 与有限 action receipt；SQLite 持有身份 namespace、directory revision、条件 mutation 与 durable delete intent；coordinator 管理 owner、advisory/enforced range。普通引用状态限于当前 authority incarnation。 | R-FS-5 至 R-FS-8、R-CC-12 至 R-CC-14、R-WS-7 |
...
基础数据操作各自完成一次请求；FileSession 保留 File、NodeReference、Use/range 与有限 action receipt，显式 S/X 另有 Session、Owner、grant 与动作历史。节点事实、opaque metadata、directory revision、pending generation 和带 owner／序号的 delete intent 由 SQLite 持久；普通引用、Scope 与 session receipt 在 authority 重启时失效。关闭响应报告引用释放事实，持久 owner 允许新会话发现未 ACK 的义务。handler 只接受 `locked.New` 验证过的配对 backend，其 `LockService()` 就是绑定原生发布检查的授权方，不能从另一份 storage 单独提供控制服务。Strong 的恢复与拒绝规则见[文件锁设计](file-locks.md)。可选 Authorizer 先按业务身份和语义决定准入，再访问 capability、Log 与 backend；通用有界请求／响应容量可先返回 EAGAIN。操作映射、策略错误与 context 生命周期由[业务授权](authorization.md)定义。
...
| 操作 | 方法与路径 | 操作数 | 请求体 |
|---|---|---|---|
| `Stat` | `GET /v5/stat` | `path` | — |
| `SetAttr` | `POST /v5/setattr` | `path` | 要改的共同属性，JSON |
| `List` | `GET /v5/list` | `path` | — |
| `Read` | `GET /v5/read` | `path` | — |
| `Write` | `POST /v5/write` | `path` | 文件内容 |
| `Create` | `POST /v5/create` | `path` | — |
| `Mkdir` | `POST /v5/mkdir` | `path` | — |
| `Remove` | `POST /v5/remove` | `path` | — |
| `RemoveDir` | `POST /v5/removedir` | `path` | — |
| `Rename` | `POST /v5/rename` | `path`（源）、`to` | — |
| `Space` | `GET /v5/space` | 无 | — |
| `Subscribe` | `GET /v5/subscribe` | 无 | — |
| `Resubscribe` | `GET /v5/resubscribe` | `incarnation`、`position` | — |
...
`Prefix` 为 `/v5/`，相对于 handler 被挂载的位置，挂到别处用 `http.StripPrefix`。v5 使用中立 Attr、metadata、目录 observation、名字 binding、guard 和 range DTO，保留 mutation barrier、配对锁授权方、原 file journal 与 ACK。server 不提供旧协议路由，client 同时验证路径版本与响应 marker；旧服务端不能通过忽略新字段接受请求。Strong 控制编码见[文件锁协议](file-locks.md#http-v5-编码)，保留文件与当前可选能力见[文件能力](file-handles.md#五http复制与资源)。

## 三、响应的形状

每个响应带两个头：

- `Remote-Fs-Protocol: 5` —— 标记这是本协议的服务端给出的答案。
...
handler 用 `MaxBodyBytes` 限制基础 volume 的 non-write 请求与 non-streaming 响应，用 `MaxWriteBytes` 单独限制 `Write` 的文件内容；package options 中后者为零时继承前者，显式值必须为正且不大于前者。过大的 `Write` 在调用 storage 之前以 `EFBIG` 返回，过大的 `SetAttr` body 是 `413`。协议定义为 bodyless 的操作只探测是否出现第一个字节，任何非空 body 都是 `400`，不会按声明长度保留内容。过大的 `Read` 是 `EFBIG`；无法在 `MaxBodyBytes` 内编码的 `List` 是 `EIO`。错误响应也必须装进 `MaxBodyBytes`，过长的 detail 会换成固定诊断，协议 marker 与 errno 不丢失。Strong 控制单独使用固定 16 KiB body；文件 metadata/range 控制使用 256 KiB envelope 与独立 operation / waiter admission，编码与预算分别见[锁协议](file-locks.md#控制准入与取消)和[文件能力](file-handles.md#五http复制与资源)。
...
**缺席与零值必须分辨得开。** `Stat` 响应里的 `attr`、以及每个目录条目里的 `attr`，缺席就是解码失败。ID 与 NodeKind 还要通过非零/枚举校验；大小、共同时间和 POSIX `000` metadata 可以合法为零。可选 BirthTime/ChangeTime 的缺席明确表示 unknown。`entries` 同理：`null` 与 `[]` 意思相反。mutation response 必须是一个只允许可选 `barrier` 字段的 JSON object：没有 change log 的 volume 返回 `{}`；有 log 时 barrier 必须存在，且带非空 incarnation 与非负 position。`null`、缺字段、未知字段或错误类型都不是成功答案。
...
`Attr.id` 是节点身份（R-FS-5）：一个不透明的非零无符号整数，只可比较相等，随节点走过改名。`Attr.kind` 也必须是已知非零枚举；两者缺失或非法都使整份响应失败。其它字段不能用零值推断是否存在：大小、共同时间和 POSIX `000` metadata 都可能是合法事实。

`Attr.kind` 使用封闭的 NodeKind 枚举。metadata 是 namespace 到 `{version,data}` 的 map，两个 byte 字段都用 canonical base64；响应 version 必须非空。BirthTime、ChangeTime、AccessTime 与 ModTime 各是 `{unix_sec,nanos}` 对象，覆盖 `time.Time` 全域；前两者可整体缺席。`AttrChange` 允许 BirthTime、AccessTime 与 ModTime 整体缺席，ChangeTime 不在请求中，由 authority 在实际状态改变时维护。
...
保留文件的响应 envelope 按操作携带 FileSession 状态、File/NodeReference 能力、原子 open outcome、属性、字节、metadata、directory observation、reference name、scope、reference state、action/delete-intent status、intent page、close release、owner、range 结果与可选 barrier，不能套用基础 mutation 的空 object 规则。Data、原始叶名、directory revision、metadata token/payload 与 generation 使用 canonical base64；时间间隔以整数纳秒编码。Open 先返回有期限的待确认能力，client 完成确认才交给调用方；未确认引用与有限 action 记录受 registry 上限约束。DirectoryMetadata 是 DirectoryReader 与 DirectoryMetadataObserver 的 v5 transport bundle bit，两个目录 facet 的完整 backing chain 缺少任一项时不宣告；Namespace 独立表示 LookupAt 与 MutateName，ReferenceName 也独立协商。任意未知字段失败。完整形状与核对边界见[文件协议](file-handles.md#五http复制与资源)。
...
HTTP replication 的 v5 Node wire 携带大小、`allocation_size` 与 `allocation_known`，不加入 DirectoryRevision。snapshot 与 change 保留来源节点的已知或未知分配事实；内置 authority 的分配量始终已知，第三方来源可以未知；HTTP-backed SQLite replica 为缺失 directory revision 的目录建立本地 opaque revision，并在 replay 名字变化时替换它。该 token 不越过 replica 边界，也不用于 NamespaceGuards；ReadDirNode、完整目录 metadata 与 reference current-name 通过独立 file operations 回到 authority。
...
`httprest.Storage.Checkpoint(ctx)` 通过 `GET /v5/checkpoint` 取得原子日志水位。响应是现有 MutationBarrier 的裸 `{incarnation,position}` object，和成功修改回复里的 barrier 使用同一 identity／position 验证；它不代表某个特定修改。请求不带操作数或 body，client／server 都使用普通请求响应预算、协议标记与封闭 errno 检查。
...
默认 List 的四倍预留是 4 GiB，8 GiB aggregate 因此只允许 2 个同时活跃；其余调用至多有 64 个等待，超过队列即以 `EAGAIN` 拒绝。并发与字节预算均可按部署或测试负载显式收紧或扩展，彼此仍独立校验。
...
基础 storage 的十一个操作、路径规则与错误词汇见顶层设计第四节。`FileStorage` 的保留对象、身份 namespace、directory revision/observation、action/delete-intent、metadata、Uses 与 range 是独立能力，原生 EX 所有权、共享预算、schema v10 和最终释放见[文件句柄设计](file-handles.md)。
...
`limited` 的额度仍按精确 payload 字节结算。它在路径、文件引用和目录观察返回的全部 Attr 中遮蔽底层分配量，报告 `AllocationKnown=false` 与零值，并以 `EOPNOTSUPP` 拒绝 `CheckAllocationReporting`。需要权威分配量的 FUSE／SMB 入口在产生任何文件效果前拒绝该包装链。

`Space` 报出的「还能写入的量」取配额剩余与底层实现所报之中较小的那个。配额是「还允许写多少」而不是「这些字节一定放得下」：底下那块盘比配额更紧时若仍报配额，等于向先查空间再决定写不写的程序许诺机器给不出的余量。

**自己记账：`packages/storage/objectstore`** 不需要被包起来。它的树存在一个数据库里，每个普通文件按 4096 字节虚拟 cluster 计入已分配额度；零长度文件、目录与符号链接为零。每次改变文件大小的 transaction 同时更新节点 `allocation_size`、volume 的 `allocated_used`、精确内容长度账 `used` 与配额裁决，不存在「判定有余量」与「占用余量」之间的窗口。`Usage()` 报告精确 payload；`Space.Used` 和逻辑 `Avail` 从 `allocated_used` 取得，包含 detached 文件。迁移后既有超额 volume 保持可读，零余量只阻止增加分配额度。没有配置配额时 metastore 以 `ENOSYS` 拒绝 `Space`。

SQLite metastore 还给 reserved、unresolved 与 garbage object records 的合计数量和 payload bytes 配置独立阈值。一个 payload 自身超过 byte threshold 时 `Reserve` 返回 `EFBIG`；请求本身能装下、但现有 backlog 使新记录越界时返回 `EAGAIN`。`Put` 失败时 reservation 转成 unresolved；这类结果没有 ownership proof，不会因为时间经过而被删除。已经存在的 volume 修改仍可产生 garbage 并把 backlog 推到阈值之上，此时新 reservation 保持拒绝，garbage 清扫与删除继续运行。package 默认值与 `-max-pending-objects`、`-max-pending-bytes` 用于 Azure 和本地形态；本地组合还通过 `Config.ObjectLimits` 暴露覆盖值，见 [`local-disk-object-store.md`](local-disk-object-store.md#七容量与资源上限)。两种 metastore-backed 形态同样使用有界 SQLite reader pool，并以独立 `MaxMetadataBytes` 限制节点、directory revision、link target 与 retained history；这些上限不计入文件内容 quota。

SQLite 打开与 `ObjectStatus` 验证每个 volume 的 named 节点形成 rooted tree：root 没有 incoming entry，其余 named 节点恰有一个同 volume 的名字且从 root 可达。detached 可以是非 root 的保留节点，但不得有 incoming entry 或作为目录边的 parent/child；detached 目录因此为空。dangling edge、具名却标为 detached 的 child、其它孤儿、cycle 与跨 volume entry 以 `EIO` 拒绝。每个目录的 revision 必须是八字节非零值，非目录必须为空；Node 与 retained change 的 kind/revision 组合一致。`volumes.used` 必须等于全部 regular-file size 的 overflow-checked 精确长度之和；`volumes.allocated_used` 必须等于全部 regular-file allocation_size 的 overflow-checked sum，包含 detached 节点。authority 节点与 retained change 的分配量按种类、长度和 4096 对齐验证；replica 保留来源已知值或未知，不强加 4096；`metadata_used` 必须等于当前、detached 和 retained change 的规范 metadata、link target 与 directory revision 长度。动态 storage class、NodeKind、可选时间、content revision、metadata envelope、trigger 定义与历史 nullable groups 都在暴露 volume、完整 listing 或 history 前验证；损坏不能被 coercion 成缺行、Detached 或可信零值。

schema v8 迁移不能为旧 retained changes 补造 directory revision。迁移事务保留当前树与身份，为现有目录建立初始 revision，清空旧 changes，并为每个 log 生成新 incarnation、归零 committed／trimmed 窗口。旧 replica 因 incarnation mismatch 重新取 snapshot；server 不允许它沿已经删除且缺少 revision 的历史续读。schema v10 迁移从当前、detached 节点与可保留的历史 change 计算各自的分配量，并重建 `allocated_used`；它保留精确 `used`，所有回填、额度与完整性校验在暴露 volume 前完成。
