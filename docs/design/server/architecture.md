...
server 通过配对的 namespace 与锁服务访问命名空间。集成方注入具有原生发布能力的实现（R-INT-6），或选择随附形态：
