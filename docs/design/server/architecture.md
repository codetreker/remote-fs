...
| **storage** | 命名空间的实际存取。由集成方提供，或使用随附的两份实现——本地目录 `packages/storage/localdir`、对象存储 `packages/storage/objectstore`；前者可以再套一层字节配额，随附的二进制在给了 `-quota` 时就这么做，后者自己记账（见第六节）。 | R-INT-6 |
