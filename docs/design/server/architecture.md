...
- 写入按新旧长度差额在底层调用前记账，超出配额以 `EDQUOT` 拒绝，写失败则撤销记账。负增量也提前释放，另一写者能使用最终失败的缩短操作所释放的额度；见[缩短提交后释放配额](../../../.agents/notes/proposed/bug-fix/2026-09-07-release-shrunk-quota-after-commit.md)。
...
- 路径 stripe 锁只覆盖操作直接指定的路径，目录改名不能保护正在写入的子路径，全部经过 server 的修改仍能少计；见[目录改名中的配额记账](../../../.agents/notes/proposed/bug-fix/2026-09-07-keep-quota-accounting-stable-across-directory-renames.md)。绕过 server 直接改动底下的目录同样会让计数漂移，这是 `docs/spec/` 里写明的非目标。计数压在零以上只约束数字范围，不能证明账本准确：从旁边拷入、再经这里删除的文件会使计数停在真相之下，此后写入可能越过实际配额。回到实测值依靠使用同一组 measurement limits 的 `Recount`，随附二进制把它接在 SIGHUP 上（见第九节）；超限、取消或 listing 失败都保留原计数。
