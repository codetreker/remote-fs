...
server 是卷、保留文件与显式占有的权威持有者：将 storage 与对应锁服务配对，经 HTTP 暴露给多个 client。角色边界与跨角色契约由 [`../architecture.md`](../architecture.md) 定义。

## 本目录的规则

- **只写 server 内部。** 不重讲系统全貌；那是顶层设计的事。
- **不假设 client 的内部构造。** 引用 client 只能通过顶层设计定义的契约——client 内部怎么组织、怎么呈现卷，与本目录无关。一旦这里出现「因为 client 会……」这样的推断，就意味着契约没写清楚，应当去补契约而不是在这里迁就。
