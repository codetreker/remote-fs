...
server 是 volume、保留节点、身份 namespace、durable 删除义务与显式占有的权威持有者：将 storage 与对应锁服务配对，经 HTTP 暴露给多个 client。角色边界与跨角色契约由 [`../architecture.md`](../architecture.md) 定义。
...
| [`file-handles.md`](file-handles.md) | 保留对象、原子子项操作、action/delete-intent、条件修改、中立 metadata、Uses/range 与文件会话协议 |
