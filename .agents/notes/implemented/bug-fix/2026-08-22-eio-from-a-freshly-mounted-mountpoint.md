...
SQLite 的纯只读取消保留 context 原因并归为 `EINTR`。普通读取、snapshot、mutation 和 Replica Apply/Seeding 都显式取得所属 pool 的 `sql.Conn`，沿用各自原有 transaction context，私有事务拥有者保留绑定连接的关闭职责。[database/sql 的 Tx.awaitDone](https://github.com/golang/go/blob/e3336a22ad3f0a90bd252c95d8b5544e02674205/src/database/sql/sql.go#L2207-L2230)在 context 取消后主动回滚，[再次 Rollback](https://github.com/golang/go/blob/e3336a22ad3f0a90bd252c95d8b5544e02674205/src/database/sql/sql.go#L2324-L2359)可在原回滚仍占用连接时直接返回 `sql.ErrTxDone`。因此事务统一收尾必须先 Rollback、再以绑定的 Conn.Close 等待连接归还；关闭只执行一次，错误保留。否则读取已返回取消时，Store.Close 仍可能因 reader 占用而得到 `EBUSY`；无见证 writer/Replica 更可能在 driver 回滚未完时关闭 store 并释放原生排他所有权。mutation 和 Replica 在连接收尾之后才释放 health/commit/Replica 门，Seeding 也在此后结束其拥有权；已经 Commit 的事务仍经过同一收尾，BeginTx 失败也释放已取得的连接。启动 schema Prepare 用相同的连接拥有权原则，返回准备结果前完成原 context 事务的 Rollback 与 Conn.Close，防止取消中的迁移仍占连接时构造失败清理已释放原生所有权。迁移 SQL、schema 版本和已有提交结果分类保持。

只读路径只有在连接关闭成功后，确切的直接 `ErrTxDone` 才能结合事务原 context 解释为自动回滚；这种取消也可能发生在查询回调成功之后。独立 Conn.Close 错误保留为 cleanup failure，即使错误看起来像取消或 `ErrConnDone`，也不能将它折叠成正常结束。清理沿用原有 transaction context，不通过屏蔽取消、轮询连接池或重试来改变请求行为。mutation、Replica 和启动准备保持既有提交不确定性、见证和清理失败规则；准备路径也只有在连接关闭成功后才接受直接 ErrTxDone，独立 cleanup 原因保留为 durability failure。读取消的分类不能扩展为写入成功或撤回证明。真实查询错误与独立清理故障不会被取消覆盖；deadline、无法命名的故障、未知 commit 或 poison 仍为 `EIO`。SQLite code 9 只在确有已取消的读取 context 时解释为取消，不能仅凭 `SQLITE_INTERRUPT` 数字推断请求已撤回。
