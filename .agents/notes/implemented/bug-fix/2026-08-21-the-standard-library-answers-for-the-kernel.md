...
`Rename` 直接调 `syscall.Rename`，失败时自己把 `*os.LinkError` 组回来。

组装由这里做还有第二个理由：`os.Rename` 组出来的那个 `*os.LinkError` 装的是宿主路径。那是服务端目录的布局，而这个错误要走到另一台机器上的调用方那里去。现在 `Old` 与 `New` 是命名空间的路径。
...
契约管不到的部分由 `localdir` 自己的用例守着。契约要在任意文件系统上成立，所以它对「目标目录里还有东西」接受 `ENOTEMPTY` 与 `EEXIST` 中的任意一个，对另外两个答案一个字都没说；`TestRenameTakesAnEmptyDirectorysPlace` 与 `TestRenameReportsTheKernelsErrnoForADestinationItCannotReplace` 的宿主是 Linux，因此可以直接钉住内核的那个答案。
...
- 走到调用方那里的 `*os.LinkError` 装的是命名空间路径，服务端目录的布局不再随错误泄出去。
