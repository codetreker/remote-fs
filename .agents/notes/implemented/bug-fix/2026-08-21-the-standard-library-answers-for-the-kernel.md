...
组装由后端做还有第二个理由：`os.Rename` 组出来的那个 `*os.LinkError` 装的是宿主路径。那是服务端目录的布局，而这个错误要走到另一台机器上的调用方那里去。该修复把 `Old` 与 `New` 设为 volume 的路径。
...
- 走到调用方那里的 `*os.LinkError` 装的是 volume 路径，服务端目录的布局不再随错误泄出去。
