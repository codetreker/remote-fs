...
于是这一版**只有 metastore 后端的 workspace 会被复制**，`localdir` 后端保持今天的直通行为。[观察源与通道](../../proposed/architecture/2026-08-19-observation-source-and-channels.md)论证过一件事：`mount(localdir)` 必须走与生产相同的缓存与失效路径，否则差分对拍对的是一个生产中永不出现的配置。这一版满足它的方式不是给 `localdir` 补一条日志，而是**把挂载层的对拍夹具换成 objectstore 后端**（SQLite + Azurite，已经能跑）。
