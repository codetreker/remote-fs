...
保留句柄每次读取都捕获同一节点的当前 `FileState`，属性、大小与 object key 共同决定该次范围和 EOF。范围写入与截断在 namespace materialization 预算内读取当前完整对象并生成下一份完整对象，在原生提交处以 `content_revision` 比较。只有明确未提交的 revision 冲突且 `Abandon` 成功时才重试；未知提交与 cleanup 故障不被重放。普通描述符的重叠写入按提交顺序各自生效，范围之外的当前字节得到保留；每次调用同步确认，关闭不承担延迟提交。这项内部 CAS 与调用方显式内容版本校验保持独立。
...
schema 的 `0003_durable_state.sql` 从 v2 前滚到 v3，建立 backing-store binding、database identity/generation、高水位、全局 identity type/max expression indexes 与带 predecessor 的 change 表。v1/v2 migration 先在同一 transaction 内验证全库结构、对象归属、序列、树与日志 tail；v2 的 retained rows 没有 predecessor，不能被宣称为已验证的连续历史，因此迁移保留全局 node/change 高水位、清空 retained changes，并为每个 namespace 生成新 incarnation、把 tail／trim 归零。迁移后的新身份与位置仍严格高于旧高水位。v2 的最终形状由[独立 v2 fixture](../../../../packages/metastore/sqlite/internal/integration/testdata/version2.sql)钉住，已落地 migration 不随当前 DDL 改写。这补上了[元数据复制](./2026-08-27-metadata-replication.md)留下的「下一个迁移必须为 v2 建历史见证」义务。`0004_lease_recovery.sql` 另增占有恢复证据，generation 与租期数值不充当 node version 或 change position。

`0005_retained_files.sql` 在 v5 增加 detached 标记与 content revision；既有节点迁移为 named、revision 从 1 开始。detached 只能是无 entry 的非 root 普通文件，named 节点继续构成根树；两者都参与对象关系、类型和用量检查。unlink 或 rename 覆盖仍有 pin 的节点时，只删除名字并标记 detached，内容保持 referenced；其后修改不进入 named change log，snapshot 只遍历名字树。最后物理关闭才在事务中删除节点、按当前大小释放配额并授权垃圾回收。独占 opener 的启动事务先验证所有 namespace，再回收旧 epoch 的 detached 节点；旧引用明确失效，另一有效 owner 被 EX ownership 排除。恢复不能因某一份 namespace 打开成功而跳过其它 namespace 的损坏。
...
本地 store 要求部署方给出至少 4096 bytes 的正 quota。metastore 的 `Used` 统计 named 与 retained detached 文件当前引用的逻辑 payload；reserved、unresolved 与 garbage 不吃 workspace 配额，但继续占物理盘，分别由状态查询报告。写入在 `Reserve` 与 `Commit` 的事务中执行逻辑配额，越过 workspace allowance 返回 `EDQUOT`。
...
native file reference 默认每 namespace 最多 65536 个，重复打开同一节点分别占名额，满额返回 `EAGAIN`。`advisory.Config` 的默认内容操作边界是单文件 1 GiB、namespace materialization 总量 2 GiB、32 个同时操作、8 次内容尝试与 30 秒单次操作期限。写入分配前收取 current + next，读取收取完整对象加返回范围；上限本身无法容纳为 `EFBIG`，当前容量占用为 `EAGAIN`。最大 materialization 必须至少容纳两份最大文件，共享 namespace 的配置必须一致。这些预算与 localdisk 的物理容量、pending ledger 和 HTTP retained bytes 分别计量，避免以范围 I/O 的小请求大小掩盖整对象成本。
