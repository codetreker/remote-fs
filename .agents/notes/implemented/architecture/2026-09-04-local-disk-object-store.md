...
`METASTORE` 把 SQLite commit 的确认边界放在 WAL 之外。它记录 accepted state A：database identity、generation、node high-water 与 change high-water，并记录已经完整进入主数据库的 checkpoint generation C。durable open 与每项 mutation 先提交 SQLite，再把下一份 A 写入并同步 `.METASTORE.stage`，以 rename 原子替换 final，最后同步 root；A 发布成功后调用才返回成功。stage 只证明见证更新开始，不是 acknowledgment。A 发布失败会 poison SQLite，后续 read、write 与 checkpoint 都以 `EIO` 停止。
