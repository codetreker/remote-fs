...
本决定取代[第一个可用版本的范围](../process/2026-08-19-mvp-scope.md)里「`statfs` 一律答 `ENOSYS`」那一条，那份 note 的其余部分不受影响；它也部分改写了[卷进入契约](../../proposed/architecture/2026-08-19-volume-in-the-contract.md)对配额归属的划法，见「配额的取值仍然不归本系统」。[移除宿主目录后端](../simplification/2026-09-08-remove-the-host-directory-backend.md)保留通用 `limited` package；随附二进制的两种存储形态由 SQLite 维护逻辑账本。[活文件句柄](2026-09-08-live-file-handles.md)将同步修改与脱离目录后的保留字节纳入同一计费边界。

## 问题

作出本决定时，每个卷都有容量上限的需求（R-WS-5），而系统里没有任何一处知道这件事。

挂载点对 `statfs` 答 `ENOSYS`：`df` 打印「功能未实现」，`stat -f` 与 `os.statvfs` 也一样。一个先问「还剩多少」再决定写不写的程序在这里问不到答案，而它多半不会因此停下 —— 它会照写不误。

也没有任何一处在数这份卷占了多少字节，更没有任何一处会因为超额而拒绝一次写入。一个卷于是能一直长到服务端那块盘满为止，而到那时倒下的不只是它自己，是同一台机器上的每一个卷。

当时随附的普通目录后端没有卷配额，R-INT-6 的集成方接入的自有存储同样可能没有。只要求底层报告和执行现成的额度，无法覆盖这类存储；这也是通用配额包装仍然有用的原因。
...
`Space(ctx) (Space, error)`，不寻址任何路径 —— 一个卷就是一个卷，「这个子目录还剩多少」是这份契约答不出的问题。`Space` 是三个字节数：总量、已用、还能写入的量。

三个数各自实测，互不推导。**还能写入的量不是总量减已用**：一份配额说的是「还允许写多少」，不是「这些字节一定放得下」，所以底下若有更紧的限制，报的是更紧的那个。少了这第三个数，一个装在快满的盘上的卷会拿着自己配额里的余量去回答，向先查空间再决定写不写的程序许诺机器给不出的空间。本地对象存储以 `f_bavail` 测量普通进程可用的磁盘容量，并扣除维护预留与在途写入；文件系统留给特权进程的容量不能计入可写余量。反过来已用可以超过总量，那是配额被下调到已写内容之下的样子，此时还能写入的量是零。

`Space.Coherent` 判定这三个数能不能同时为真：都不为负，且还能写入的量不超过总量减已用。判不成立的答案是一次失败，不是一组待修的数字 —— 它们要进内核回复的无符号字段，一个负数在那里是一个巨大的正数（R-ERR-2）。

**自己没有容量可报的卷答 `ENOSYS`，而且这是实现的固有属性，不是这一次调用的状况**：会答的一直会答，不会答的从来不答。上层因此既不把它当作可重试的暂时故障，也不替它推算一个数字，还可以在收到一次之后再也不问。
...
- **让卷变小的修改从不被拒绝**，已经超出配额时也不拒绝，否则一个超额的卷没有任何回到配额之内的路。同理，一份已经装得比配额多的卷照常打开 —— 把配额调到已写内容之下是运维日常，答案是「在吐出一些之前不再收新的」，不是「这份卷没法服务了」。
...
卷操作与同步句柄修改持有 Recount 闸的共享访问，Recount 独占它。引用到期和最后关闭的清理仍可推进；它们的计费不能在原生发布锁内等待该闸。Recount 先记下计费 revision，再测量权威用量或执行有界遍历，仅在 revision 未变时安装完整结果；清理交错时重新测量，连续八次都交错则返回 `EAGAIN`。越界、取消或 backing error 不安装测量值，已经确定的并发结算仍保留。

发布效果未知，或结算、撤销返回 `IsPublicationAccountingUncertain` 时，即使卷已知没有改变，也停止 Space、修改与 Recount，并保留首个不确定错误及原因，直到重新打开。普通准备失败且成功撤销保持可用；不能用一次在线重数掩盖未决计费的后续效果。
...
`packages/storage/objectstore` 采用同一条组合规则，但逻辑账在 metastore，物理余量来自 `Objects.Available`：`Space` 的 `Total` 与 `Used` 保持卷口径，`Avail` 取 metastore 余量与 object store 实测值的较小者。只有纯粹的 `ENOSYS` 表示实现没有物理数字；与 I/O failure 合并的 `ENOSYS` 仍然是失败。随附二进制的 local-store 与 Azure Blob 形态都使用 SQLite 事务中的额度与计数，不再套一层 `limited`。`packages/storage/objectstore/localdisk` 从 `f_bavail` 扣掉 maintenance reserve、在途 publication 与格式开销，具体取舍见[本地磁盘对象存储](./2026-09-04-local-disk-object-store.md)。
...
挂载点按 4096 字节一块报出容量。卷按字节计量，块只是报出去时的计价单位，4096 是拿来跟它比较的那些文件系统用的数。
...
三个块数一律向下取整：装不满的一块不拿出来给人。inode 那两个数是 0，因为这里按字节收费、根本不数 inode，填任何数字都是编造；0 是「没有 inode 表」的报法，`df` 把它显示成没有这一栏。255 是卷可能落在的 Linux 文件系统里最短的那个上限，报小了是安全的一边：一个先问名字装不装得下的调用方，会被告知一个本来装得下的名字装不下，而不是反过来。

于是**配额不得低于 4096 字节**，`limited.New` 当场拒绝。一个比一块还小的配额会被报成一个零块的文件系统 —— 而那正是 `statfs` 此前答 `ENOSYS` 所要躲开的那个全零回复：它读起来是一块没有剩余空间的盘，而不是一个空间很小的卷。

三个数不能同时为真时是 `EIO`。两者都不换成编造的数字。

### 配额的取值仍然不归本系统

设定配额是部署方控制面的事，与计费、归属、生命周期策略在一起 —— [卷进入契约](../../proposed/architecture/2026-08-19-volume-in-the-contract.md)把这些划在系统之外，那一条仍然成立。**报出它与执行它则是本系统的**：一个只由部署方记在别处的数字，落到挂载点上的程序那里什么都不是。
...
**它对不经过这里的修改一律不精确，而且两个方向都错。** 绕过 `limited` 直接修改其底层卷，是明确的非目标；这不是疏忽，是因为**没有任何经过包装层的流量能把它修回来**：

- 一个文件从挂载点写进去、从服务端那侧被删掉：那些字节永远挂在账上。而那个路径已经不在了，此后不会有任何操作再碰到它，于是也不会有任何一次修改把它减掉。一个构建产物走十遍这个循环，就能让一个空目录上的卷卡在 100%。
- 一个文件从服务端那侧被拷进去、从挂载点被删掉：账上被退回它从没收过的字节，计数因此**停在真相之下**，不加约束还会一路变负 —— 而一个负的计数会让「还能写入的量」大于总量，挂载点开始宣传一份并不存在的容量。

于是有两件事，缺一不可。

**算术被守住，让报出去的数字造不出容量。** 计数在存的地方就被压在零以上，不是只在读的时候才压；报出的「还能写入的量」于是既不超过配额剩下的部分，也不会从内核回复的无符号字段里出来变成一个巨大的正数（R-ERR-2）。**被守住的到此为止 —— 是报出去的数字，不是账本本身。** 计数照样可以停在真相之下：一个从旁边被拷进来、再经由这里被删掉的文件就留下这样一笔，此后的写入照单被接受，卷越过配额而没有任何人被拒绝。压在零以上挡不住这件事，只是不让它再从报出去的数字上多骗一份容量。对经过这里的修改，拿不准时一律往「多收」的一边取 —— 一个 stat 不出来的文件被删除时不退款，一次 `O_TRUNC` 之后的写入按整份长度收费而不是按增量 —— 但这条偏向只管得住经过这里的流量。
...
**配额由底层存储自己执行，本系统只把它报出来、并把它的错误原样透传。** 不用自己数，也不会漂移 —— 数字来自唯一有资格给出它的那一方。当时否决这项统一前提，是因为随附的普通目录后端没有卷配额；集成方自有的存储同样可以没有（R-INT-6）。这条路要么把可用范围缩到「底下必须自带卷配额」，要么就得在每一种没有配额的底层旁边再造一份。通用包装让这项责任有一个可复用的实现；具有事务账本的对象存储组合则直接执行自己的额度。
...
**按路径记账：一张从路径到「这个路径收了多少字节」的表。** 它有一个真优点，是 O(1) 计数器给不了的：**任何被再次碰到的路径都会自我纠正** —— 下一次写它、删它，都会拿表里那个数去抵，从旁边改动造成的偏差当场结清。输在两处。一是 R-INT-3：一个文件一条记录，就是随卷无界增长的每节点状态，而 R-SCALE-1 明说单个卷的体量还没实测过。二是它救不了真正把卷卡死的那种情况 —— 从旁边被删掉的那个路径再也不会被碰到，它那条记录会永远留在表里，和一个 O(1) 计数器里那笔永远退不掉的账一模一样。
...
- 挂载点上的 `df`、`stat -f`、`os.statvfs` 报出的是实测的数字，一个先问再写的程序拿得到答案。契约用例守着它：`Space` 只有两种被允许的结果 —— 一份能同时为真的报告，或者 `ENOSYS` —— 没有第三种，且这两种之间的选择在同一个卷的一生中不许改变。
...
- 对象存储组合把逻辑额度与 backing store 的实测物理余量分开保留，再以两者的较小值回答可写量；卷用尽仍是 `EDQUOT`，底层磁盘无法容纳 publication 是 `ENOSPC`。
- 第三方实现（R-INT-6）多欠三条义务，全部可执行：要么答容量、要么答 `ENOSYS`，绝不报一个推算出来的数字；三个数都不为负、且还能写入的量不超过总量减已用；答不答这件事不随卷的生命变化。

### 付出的，以及必须叫出名字的缺口

- **计数是 O(1) 的，代价是它在卷被从旁边改动之后两个方向都错。** 多收会把空目录记为满额，少收会让后续写入超额；压在零以上只约束数字范围。原生发布计费将受管修改的收费与实际目标绑定，第三方路径采样的[目录改名交错](../../proposed/bug-fix/2026-09-07-keep-quota-accounting-stable-across-directory-renames.md)仍是缺口。正常账本可以通过成功的 Recount 重新实测；发布或计费结果不明的账本必须保持不可用。
...
- **没有权威 Usage 的底层要在构造 `limited` 时遍历一次卷。** 遍历的 retention 由两个默认 64 MiB 上限约束，但工作量仍随文件数量增长；一个 directory 或 frontier 超过配置会以 `EIO` 拒绝整个构造，不会交出 partial count。支持保留句柄却缺少权威用量时直接拒绝组合。
