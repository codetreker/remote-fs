...
只比较 `committed_position` 与最新 surviving position 能发现尾部丢失，不能发现中间缺行。position 又由整个数据库共享的序列分配，同一个 namespace 的相邻 change 之间合法地夹着其它 namespace 的位置，因此数值不连续不是损坏，不能用 `position + 1` 判定。
...
每条 `changes` row 保存 `previous_position`，指向同一 namespace 的上一条 change position。追加 change 时，在同一个 SQLite transaction 中读取当前 `committed_position` 作为 predecessor，显式分配新的全局 position，插入 row，再把 committed tail 推进到新 position。
...
schema v2 没有 predecessor，不能把旧 retained rows 回填成已经被证明连续的历史。v2 迁移先验证旧 row 形状、tail、trim 与 sequence，保留全局 node/change 高水位，然后删除 retained changes、为每个 namespace 生成新 incarnation，并把 committed/trimmed position 归零。已有 replica 通过 incarnation mismatch 重建；迁移后的第一条 change position 严格高于旧高水位。
...
**要求同一 namespace 的 position 数值连续。** 不增加 schema 字段。输在 position 是数据库级全序；其它 namespace 的合法提交会在本 namespace 相邻 row 之间留下空洞，算术连续会拒绝健康数据库。
...
- 完整链验证与一份 namespace 的 retained log 大小成正比，受 `MaxIntegrityRecords` 限制。合法但超过 serving ceiling 的 namespace 在 open、snapshot 或 status 时以 `EFBIG` 停止，必须显式提高上限；`Since` 保持每页 O(page) 工作。
