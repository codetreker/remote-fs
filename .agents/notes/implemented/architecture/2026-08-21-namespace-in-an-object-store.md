...
`localdir` 与 `limited` 仍是命名空间的合法持有方式。本地磁盘实现的格式、持久化与组合所有权由[本地磁盘对象存储](./2026-09-04-local-disk-object-store.md)记录；[显式文件占有](./2026-09-07-file-locks.md)把 authority 绑定到 metastore 的原生最终发布。两者扩展这份两层结构，树与对象继续各自持有名字和字节。
