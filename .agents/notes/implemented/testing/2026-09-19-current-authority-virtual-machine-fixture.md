...
mapping 脚本在 after-readline 与 before-json 之间观察原 ReadLine 值，不修改字符串：null/empty/value、UTF-16/重新编码 UTF-8 长度和 SHA256 与父端不含 LF 的 line hash 比较，原 framed input hash 另保留。先限制 4096 UTF-16 和 12288 UTF-8 字节再分配，超限明确以 input-observer 失败，不补造 digest。三个 child-effective 环境值只经 .NET 各读取一次，记录状态、长度与至多 96 字节 base64 前缀，和父继承、配置输入分开，不把它们视为模块解析证明。

catch 先保存原 ErrorRecord，再于 JSON 错误序列化之前写 catch-entry、原 stage、异常 type 与 error ID 的有界事实（各至多 64 字节前缀）。输入/环境观察失败本身是对应阶段的主失败，不能继续操作成功；观察已有异常时发生的错误是次要错误，finally 仍以保存的原 ErrorRecord 尝试原序列化与失败退出。begin/end/failure marker 使观察阶段和操作阶段分开，缺席完成不能解释为成功。各分支最大 formatter 的合成上界含原 marker、Utility 前后 marker 与 CRLF 为 2040 字节，沿用 2048 字节捕获前缀和原四秒期限；PowerShell 自有额外输出仍可能截断，框架初始化与 console write 不承诺独立可取消或固定时延。原 mapping 脚本的观察不增加 JSON cmdlet、Get-Command、import、预热或新进程，原错误/清理拥有权保持；后置控制的主动命令行为另行标明。

普通 mapping 命令与后置 inventory 3/4 共用 prepared executor：在原四秒 context 中核对固定 System32 Utility manifest 的路径/无 reparse/regular-file，并按既有 1 MiB 上限读取、hash 和 checked Close，随后以剩余 deadline 启动原进程拥有者。准备不发生在 backend constructor，不另起 deadline；失败保留文件和清理原因，无 child 时不制造 PID/Launch、输入/script 或 worker 事实。命令诊断里的可选 path/size/read count/hash/duration/cleanup 是磁盘准备事实，不是加载依赖字节的证明。

mapping 脚本在输入/环境观察后，以 Core Import-Module/PassThru 加载验证过的绝对路径并把结果收在局部数组，after-utility 紧跟导入返回。必须恰有一个、名字为 Microsoft.PowerShell.Utility 的 PSModuleInfo；再从其 exact ExportedCmdlets[ConvertFrom-Json] 取得非 null、名字正确且种类为 Cmdlet 的 CmdletInfo，保留该对象。JSON 前的类型/数量/name/export 错误直接失败，不重新按名字寻找或回退；模块输出不泄漏到 stdout。

原 ReadLine 字符串仍按值经 pipeline 输入，但 target 绑定为 `$request = $requestLine | & $jsonCommand`。before-json 表示 module/export guard 已通过，after-json 表示该对象调用返回；没有增加 marker、field、deadline，原 2040 字节合成上界与 2048 前缀保持。对象绑定不是依赖代码 hash/签名证明，也不证明原来存在遮蔽或作用域缺陷。不带 Force、不做 Get-Command，不新增进程、重试或预热；SmbShare、mapping 操作、stdin/环境、原 catch/清理拥有权保持。import/property/invocation 失败保留原 ErrorRecord，准备 Close 失败也不能因未启动 child 就变成清理成功。四秒仍共同覆盖准备、启动、导入、选择和执行，不新增同步调用的可中断或时延保证。
...
[启动诊断](../../../../.github/scripts/native-current-authority/mapping_startup_diagnostic_windows_test.go.txt)只在真实 cold 首次失败之后运行，先核对同 source/run/nonce 的初始 inventory command timeout、无 mapping/SMB/原生文件效果及清理已完成、worker 全部收齐；首行必须确为 context deadline exceeded，不能在子输出中搜索该词代替原因。带专用 tag 的测试不进入普通单元清单或生产程序；六个顺序单元各运行一次。entry 1/2 与控制 5/6 的脚本和输入策略保持；inventory 3/4 使用普通 mapping 相同的 prepared executor，保留 open/EOF 策略并绑定新的脚本 hash。第五项 exact-name Core Get-Command 解析 ConvertFrom-Json，第六项在已核对绝对路径导入 Utility 后执行原 JSON pipeline。单元有显式 kind/result，checker 核对一个根加六叶的精确 start/verdict。每项沿用四秒命令与既有清理；完整写入后才可由同一 stdin owner 发 EOF。已知清理完成的失败不阻止后续诊断，未知 handle/Job 静止状态阻止再启动；原 cold 和较早单元的失败始终保留。

解析控制不带 All、ArgumentList 或 command-type filter，只有确切一个、名字正确的 Cmdlet 结果才读取其 implementing type/assembly/MVID 和 nullable module metadata；不追随 Alias/Function，也不使用 CommandInfo wrapper assembly 冒充实现来源。Get-Command 可触发 autoload，导入可执行模块脚本或其它 I/O，不能叫作被动无效果观察。这些操作只在原 cold/VM 清理尝试之后运行，不预热原测量。

第六项仅在第五项已收齐后，才从原验证的 System32 Modules 根定位 Utility manifest，并在该项四秒 deadline 内进行无 reparse/regular-file 检查和至多 1 MiB 读取。size/hash 是当时磁盘准备事实，不证明加载字节。读或 Close 失败保留，第六项不提前准备或使用 fallback；entry 与 resolve 不要求该文件，普通 mapping/inventory 则各在自己的 context 内准备。同步文件调用不承诺 context 可中断。导入只用 Core Import-Module 的绝对路径、PassThru、不带 Force，核对一个预期 module 及其 exact exported ConvertFrom-Json；随后未限定名称的原 JSON pipeline 返回，也不能证明选中了该 export 或完整请求已经校验。

两个新控制通过 Console 输出固定 resolve-ok/import-parse-ok JSON 字面量，错误路径不调用 JSON serializer；catch 保留原 ErrorRecord、报告有界 stage/type/ID，finally 失败退出。可空 provenance 的六项字符串、实现 MVID 与有界前缀只是 metadata，不是 file hash、签名或原 cold 命令选择证明。各分支最大格式合成投影 2008 字节，仍使用 2048 字节 stderr 前缀；PowerShell 自有输出另按原限额处理。

准入不依赖输出为空：stdout/stderr 的非负 byte count、规范 base64、恰为 min(count,2048) 的前缀和准确截断标记必须一致，合法的空、部分或带阶段内容均保留。已知顺序 marker 可归类，未知尾部和半行不推断阶段；它们不替代来源绑定、初始超时与清理完成等准入条件。存在的原输入/script 事实必须匹配，缺席不补造。源码 manifest 统一生成严格 POSIX-relative key，并核对原 hash；Windows 分隔符不能作为另一套兼容 key。
...
固定 `native-evidence/mapping-startup.json` 位于 checker output 下一个本次独占创建的受保护子目录。既有 private helper 先以 owner/SYSTEM protected DACL 创建目录，再独占创建文件、seal 后写入/sync/close；它不更改 checker 外层 ACL，也不接管、截断或删除已存在的目录/文件。所有 write/sync/close 错误保留，失败产物继续作为证据。checker 只读这个固定嵌套路径，tagged receipt 含 LF 至多 1 MiB，与既有 checker reader 一致；精确上限通过，多一字节失败并保留原错误，不截断或丢单元。命令捕获与执行器限额不变。

Windows 进程拥有者记录 CreateProcess/Job assignment/ResumeThread 的时长与错误，要求实际 previous suspend count=1，并从原保留 handle 读取实际 executable 与 process/native machine。同步 native 查询不承诺 context 可取消；唯一 waiter 仍拥有 handle 关闭。各单元收齐进程和三个 worker，记录实际创建次数、次序、输入/script hash、输出与清理。全部尝试之后才 hash/检查磁盘上的 PE，不把它当作历史加载字节证明。控制脚本不创建/删除映射或 intent，不请求目标文件操作；模块发现/导入可以有模块定义的 I/O 副作用；其运行已在 VM/SMB 清理尝试后，负载和先前启动次数不同，后续成功不能独立定位原 cold 原因。120 秒测试和十五分钟作业期限保持。
...
对象绑定版本的 mapping 普通/race 各 39 根/159 verdict、tagged 准备/诊断各三根通过；二十项实际本地 PowerShell 7.5.3 AST 场景验证真实 module/export 对象、同名 sentinel 下的对象调用、旧名称目标的因果失败，以及 guards/stdout/错误拥有权。wrong-kind CmdletInfo 只由源码变异覆盖，不虚构 runtime 实例。2040 字节/33 个 CRLF 是有界 formatter 投影，三项 ARM64 构建/vet/格式检查已核对。未变的控制按原 source/graph 复用，完整范围见[测试策略](../../../../docs/testing.md#当前源码-smb-的单对象-cold-检查)。局部绑定证明不解释真实 CI 是否有同名遮蔽，不替代新源码原 cold 的 mapping/nativefile/完整清理结果；既有失败不追改。
