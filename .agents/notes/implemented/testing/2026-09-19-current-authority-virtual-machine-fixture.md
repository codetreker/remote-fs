...
第六项仅在第五项已收齐后，才从原验证的 System32 Modules 根定位 Utility manifest，并在该项四秒 deadline 内进行无 reparse/regular-file 检查和至多 1 MiB 读取。size/hash 是当时磁盘准备事实，不证明加载字节。读或 Close 失败保留，缺失路径不阻止前五项，不用 fallback；同步文件调用不承诺 context 可中断。导入只用 Core Import-Module 的绝对路径、PassThru、不带 Force，核对一个预期 module 及其 exact exported ConvertFrom-Json；随后未限定名称的原 JSON pipeline 返回，也不能证明选中了该 export 或完整请求已经校验。
...
host setup 的进程拥有者、mapping、tagged 纯逻辑与私有 writer 普通/race 控制均通过；保留旧继承模块策略、破坏 seal-before-write 次序与读取旧顶层 report 的隔离对照分别命中断言。二十七项 Python checker 用例、五项 ARM64 程序/默认与 tagged 测试构建及 vet/格式检查通过，完整范围见[测试策略](../../../../docs/testing.md#当前源码-smb-的单对象-cold-检查)。这些执行是可移植提取逻辑与受控 child 模型，实际 Windows DACL 与 Go-child 交付另由上述 26ba 运行证明，cold 效果仍未通过；先前四个叶用例通过、汇总写入失败和原 cold 失败分别保留。 新增观察的 mapping 普通/race 各 28 根/88 verdict、消费端各两根通过；本地 PowerShell 7 实际 helper/catch/formatter AST 与原/次要异常控制、两个因果变异和三项 ARM64 构建/vet 已核对。1981 字节是各分支最大格式的合成投影，不是原生 mapping 输出；这些本地检查与上述 a5ad 的实际观察分别归属，不替代新增第五/六控制的原生结果。 后置命令控制的 mapping 普通/race 各 33 根/103 verdict、准备/回执两根通过；本地 PowerShell 7.5.3 的十八项实际 AST 场景、57 项 helper 断言使用受控 discovery/import 对象，lineage 与提前准备的因果对照、二十九项 checker 用例及三项 ARM64 构建/vet 均核对。2008/935 字节是 formatter 的合成最大分支投影；不证明 Windows 5.1 自动加载、原 cold 原因或新增控制的原生结果。
