...
mapping 脚本在输入/环境观察后、原 before-json 和 JSON pipeline 前，以 Core Import-Module 加载这个验证过的绝对路径，成功输出被抑制，原 stdout result 不变。before-utility/after-utility 两个 marker 纳入同一 2040 字节合成上界，仍在 2048 前缀内。不带 Force、不做 Get-Command，不新增进程、重试、预热或按名字回退；原 ReadLine、JSON pipeline、SmbShare import、mapping 操作、stdin/环境策略保持。导入失败是真实 pre-JSON 失败，原 ErrorRecord 与错误序列化继续保留；准备 Close 失败不能因未启动 child 就变成清理成功。此修复加载同一进程所需模块，不替换 parser，也不承诺同步文件调用可中断或新的时延上界。
...
[命令控制运行 35454012237](https://github.com/codetreker/remote-fs/actions/runs/35454012237)绑定 `7bef4859bc1cbde96bc2b97cf79d018f94fcf228`，原 inventory 仍在 4009 ms 失败，exact-name Core Get-Command 在 4018 ms、resolution 返回前超时；显式 System32 Utility import 与原 JSON pipeline 控制在 1437 ms 通过，约 19 ms 的 manifest 准备包含在同一四秒内，清理非 forced。它支持显式导入路径，但顺序预热、控制上下文与负载不同，不能据此断定原超时的唯一原因，更不修改原 cold 失败。修复后的普通 mapping/inventory 尚未原生执行，仍须证明实际 mapping/native file 和完整清理；后置通过不能代替它。
...
显式 Utility 修复的 mapping 普通/race 各 38 根/139 verdict、tagged 准备/诊断各三根通过；八项实际本地 PowerShell 7.5.3 AST 场景验证导入、stdout 抑制及原/次要错误，Go/PowerShell 导入遗漏对照命中断言。2040 字节/33 个 CRLF 是 formatter 各分支与合成最大事实的投影，三项 ARM64 构建/vet/格式检查已核对。未变的 checker、进程和私有 writer 控制沿用各自收据，完整测试范围见[测试策略](../../../../docs/testing.md#当前源码-smb-的单对象-cold-检查)。这些局部检查、既有原生 DACL/环境事实与后置 import 控制通过分别归属，均不能替代修复后原 cold 的实际 mapping/nativefile/完整清理结果；既有失败不追改。
