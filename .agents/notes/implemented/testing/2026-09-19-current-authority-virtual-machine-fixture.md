...
current-SMB cold 的本地最终 helper 普通/race 各 83 根/432 verdict，可移植 mapping 控制 13 根/43 verdict；真实核心 pool 与错误 SID 的负向对照、Python/PowerShell source 检查及 Windows 构建/vet 均有独立收据。完整本地 artifact 的 265 项输入/16 项产物与冻结 README、源码一致，kernel/init/authority 字节保持原 F 组合；其 dirty=true 来源不当作未来 CI commit。这些本地验证与下面的原生 cold 结果分别计证据，不能证明暖缓存。

当前 native mapping 的可注入普通/race 控制各 25 根/141 verdict、五项因果对照及 ARM64 程序/完整单元构建、vet/格式检查已通过；Windows 实际磁盘 publish-then-EIO 和 private claim/lock 用例在本地阶段仅编译，随后在首次 WNet 原生运行中实际通过。支持层 Linux probe 各 35 根/215 verdict、本地 Unix guard 37 项与 checker 7 项分别计证据，不冒充 Windows WNet、DACL/锁或 provider-options 接受性。真实 mapping 与 sentinel 访问只属于原 cold 阶段；这些检查不能预热或替代它。

[最后一个解释器版本的首次运行 35459298859](https://github.com/codetreker/remote-fs/actions/runs/35459298859)绑定 `58d1663c19d675817d25154be8e35e6a35726c03`，原 inventory 4010 ms 失败且 stdout/stderr 均为零，没有 entry；实际 system ARM64 executable、resume1 与 219 字节写入已记录，但不能据此断言已到 import/object/native-file 阶段。后置对象 inventory 3793/947 ms 通过，discovery 4020 ms 失败、import/parse 480 ms 通过；负载/顺序变化不解释原 cold。该解释器路径的失败保持原结论。

[首次 WNet 原生运行 35471060655](https://github.com/codetreker/remote-fs/actions/runs/35471060655)绑定 `8b403735f2e87a00babbddc0c2bb2383ad40dfd0`，在 fresh hosted ARM64 上完成单对象 current-SMB cold 与 authority/VM 生命周期。create/remove 返回零且 helper 静止，owned port 62811 上的 52 个 post-auth signed packets 与 lineage、HTTP/SMB/native 的 173 字节、EOF 和同一 HANDLE 重复 FileIdInfo 均匹配；原 live attempt 完成映射删除，普通 authority 重启后以新 epoch 重开同一对象，QEMU 自然 exit0、非 forced Job 清理及私有磁盘删除/基底不变成立。probe 69 根/379 verdict、controller 62 根/254 verdict 与 40 项 Windows 路径检查通过，包含上述两个实际文件系统根。API 精确参数由源码/binary 绑定，成功 raw syscall/helper frame 未保存；signed flags/lineage 已观察，签名加密验证不冒充独立重放。该结果只证明这个 fresh runner 上的配对和单对象路径，`native_acceptance` 仍为 `not-run`，暖缓存、replacement identity 与跨对象唯一性仍未验收。
