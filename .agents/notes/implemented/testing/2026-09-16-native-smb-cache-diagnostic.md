...
early+QFid 的本地适用控制 82 根、普通/race 各 635 verdict 通过；移除早期能力位拒绝和新 CREATE 未请求条件的两个隔离对照分别命中断言。三十三项脚本策略控制与六种 ARM64 配置构建通过，五个既有分支的准备结果及 254 份非测试 Go 输入保持不变。Linux 子进程控制和精确提取的 parser 与原生结果分别计证据。[early+QFid 原生运行 35439336266](https://github.com/codetreker/remote-fs/actions/runs/35439336266)已证明 root Fs5=0x406（序号 34）< helper 完成退出（36）< HA API（38），并证明首个新 CREATE 未请求却收到真实 B QFid=4；两份证据绑定同一 HA/tree/volume。Hnew 报 native ID=4，保留 HA 却由初始 3 变成 4，仍读 A257，Hnew 读 B769。完整 DETAIL、冷 B、oracle、原设置与清理有效，所有身份/字节检查在 ACK 后 39.92 ms、TTL 前完成，结果是合格 retained_identity_changed。这个首样本不证明问题持续超过一秒；未请求 QFid 的符合性、内部 FCB 机制和生产修复仍未确定。
...
唯一管理字段佐证的本地模拟控制共 107 项（zone 39、capture 22、origin 24、controller 22）通过；关闭新分支与丢弃 provenance 的两个对照命中断言，原 raw scope0/local scope16/Equals=false 不改写。该 zone 佐证变更保持 native helper、生成结果、binary 与进程拥有者，复用既有 Go 普通/race 和 ARM64 证据；新的 share2 对照另有生成观察变化，不沿用旧 binary 相等结论。[原 share0 运行 35449828229](https://github.com/codetreker/remote-fs/actions/runs/35449828229)已完成 Ready1 来源准入，source B 的 DELETE/share7 打开成功，实际 FileRenameInfoEx22/Flags3/RootDirectory NULL/绝对 target 返回 sharing-violation 文本；没有 ACK 或 Hnew，结果为 reference_mutation_refusal，没有 native identity verdict。该旧回执没有数值 rename errno 或阻塞 handle，不能补造它们；native/controller 清理完成不消除失败。[share2 首次运行 35453426310](https://github.com/codetreker/remote-fs/actions/runs/35453426310)绑定 `bda555c01617828fb49ab8ac61235cdbed4241b2`，实际 LIST1/share2 应用打开与同一个 local FileRenameInfoEx22/Flags3 替换成功，两次来源核对及正常清理完成。首次 Hnew 读取 B769，保留 HA 读取 A257，但初始 HA、Hnew、保留 HA 报告相同 native volume/index；local NTFS 的 A/B 身份不同，后置 target 的身份与字节属于原 B。原生身份/字节检查在本地 ACK 后 2.1066 ms、最早 TTL 前完成，结果为 share_write_control_native_behavior_failure，原 NativeOracle 为 replacement_identity_aliased。正确字节与成功 rename 没有满足旧引用稳定且新对象不同的身份判据。

这个合格样本证明 share2 配置下的早期 legacy 身份 alias，不证明它持续超过一秒。通知仅记录一个 MODIFIED v 批次，没有完整 wire/lease/break、自动 B-coldness 或具体 FCB 机制证据，不能与原型的精确替换通知视为相同刺激。原 share0 的 reference_mutation_refusal 保持失败；该对照不是原场景或生产修复。当前身份要求与判据保持，系统自带栈的相同结果也不将项目验收改判通过，更不推出所有 SMB 方案不可能。
