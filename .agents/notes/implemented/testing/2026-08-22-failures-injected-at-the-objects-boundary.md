...
[放进对象存储的命名空间](../architecture/2026-08-21-namespace-in-an-object-store.md)正是这样一份实现：
一棵树加一个对象存储，两者各自会失败。这条保证要求的是**不管对象存储那一半报什么，命名空间都不得
把它答成一个关于名字的事实**。
...
换掉了：那套用例说的是「一个装在真 blob 容器加真 SQLite 里的命名空间，行为和别的命名空间一样」，
...
- **三个方法乘四种错，全部在组合层上被断言。** 按 `-coverpkg` 全模块口径量，`discard` 从 72.7% 到
  90.9%，`Sweep` 从 73.3% 到 80.0%，`Write` 从 87.5% 到 93.8%。
