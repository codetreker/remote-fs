...
**为授权单独定义打开意图和动作映射。** 可使 authz 只依赖标准库，但 FileOpenOptions、wire dispatch 与授权输入会各自保存同一份事实。新增字段或动作时，逐字段复制与分支映射容易漏掉同步。操作属于 volume 服务，打开意图属于文件访问；将二者放在基础 storage，使执行和策略读取同一定义，不引入具体 backend 或 HTTP 依赖。统一 Op 前缀将操作常量与 FileRead 等结果类型分开。[中立元数据与访问控制](../architecture/2026-09-16-neutral-metadata-and-access-controls.md)沿用该边界，不增加业务身份模型。
