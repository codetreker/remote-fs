...
文件 wire 使用规范 Operation；NameCommand 映射到实际 volume 操作，Range Apply/Drop 分别授权。普通与强锁 URL 在路由规格中声明语义操作，传输名称不承担角色含义。共享 OpenAccess 表达读、写、创建、截断、排他意图；OpenAt 的替换、armed intent、reset 和条件修改还核对其必需额外效果，全部 callback 在 native 效果前完成。锁模式不充当内容权限：只读 fd 的 EX flock 是合法操作，范围 Drop 和其它 cleanup 独立可控。volume.write 自身可能创建文件，不能只拒绝 create 就宣称禁止创建。
...
**为授权单独定义打开意图和动作映射。** 可使 authz 只依赖标准库，但 FileOpenOptions、wire dispatch 与授权输入会各自保存同一份事实。新增字段或动作时，逐字段复制与分支映射容易漏掉同步。操作属于 volume 服务，打开意图属于文件访问；将二者放在基础 storage，使执行和策略读取同一定义，不引入具体 backend 或 HTTP 依赖。统一 Op 前缀将操作常量与 FileRead 等结果类型分开。[中立能力](../architecture/2026-09-16-neutral-file-capabilities.md)沿用该边界，用固定语义映射复合效果，不增加业务身份模型。
