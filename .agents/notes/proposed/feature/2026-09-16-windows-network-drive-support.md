...
支持日常可读写目录树所需的打开与六种存在性／替换结果、读取、范围写入、截断、刷新、关闭、目录操作、名字变更、受支持属性与时间、目录通知、共享模式、范围锁及空间查询。未支持的信息类、控制操作或标志明确失败，不返回占位数据。
...
| WN-09 可见性与通知 | Windows A 分别预热正／负查找、内容、大小、属性、目录与标识；先在超过所配置静默窗口的空闲期证明没有周期性 authority 查询，再让 Windows B、Linux、SDK 各完成写入、增长、截断、创建、rename、replacement、delete。每次记录确认时点，一秒后只发起一次对应观察；另只切断 A 的变更通道而保留普通 authority 访问 | 第一次观察得到权威状态或明确 I/O 失败；同机第二进程立即可见；rename 通知成对；不能靠后台轮询。把通知上限配置为 N 后以持续 churn 产生第 N+1 个未消费事件，必须明确 overflow；恢复期间不从不完整状态成功回答，并在纳入枚举期间变化后完成权威重取或明确预算失败 |
...
| WN-15 排除项 | 对 ADS、ACL 编辑、hard link、reparse 创建／遍历、稀疏／压缩／加密、durable／persistent handle、multichannel、failover、离线写入逐项发起原生请求；快照驱动安装、LAN 监听与全局缓存／安全设置前后状态 | 每个可调用项返回明确 unsupported 且 authority 无部分效果；旧平台拒绝发布；没有驱动、LAN 暴露或全局策略变化；断线后旧句柄持续失败且没有重放 |
