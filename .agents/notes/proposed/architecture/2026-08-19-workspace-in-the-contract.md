...
发布实例还必须与约束该 namespace 的锁授权方配对。Session、Owner、Grant、逻辑资源与动作历史都属于这个授权方；不能把一个实例上的 proof 用到另一个实例，也不能仅凭同名 workspace 重放旧授权方的动作。精确结果核对限于原授权方及其活跃 Owner / Session 的历史窗口；重启后旧意图明确返回退役或未知，已确认保护通过恢复屏障继续满足。
...
**多实例的全局资源限制仍未定。** 文件锁已在单个授权方内限制 Session、Owner、资源、动作、grant 与等待，并对 Session / Owner 再设内部上限；这不等于一台 server 的多 workspace 总量已有约束。日志保留窗口按实例隔离，新增注册表时仍须定义跨实例的入站并发、内存与锁状态总量，不能让每实例有界掩盖实例数无界。
