...
func retainedOpen(t *testing.T, session storage.FileSession, path string, create bool) storage.File {
	t.Helper()
	file, err := session.OpenFile(t.Context(), path, storage.FileOpenOptions{OpenAccess: storage.OpenAccess{Read: true, Write: true, Create: create}, InitialMetadata: map[string][]byte{"test.initial": {0, 0xff, 1}}})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
...
func TestRetainedControlsRemainAvailableWhenTheStreamFails(t *testing.T) {
	...
	if err := file.Close(t.Context()); !errors.Is(err, syscall.EIO) {
		t.Fatal("close reported replica visibility while the stream was unavailable:", err)
	}
	s.events.mend()
	requireHolding(t, mounted, "file")
	if err := file.Close(t.Context()); err != nil {
		t.Fatal("close retry did not confirm cleanup after stream recovery:", err)
	}
}
...
func TestPublicListingsEnforceCurrentAuthorityDirectoryClaims(t *testing.T) {
	served := serve(t, httprest.DefaultLimits())
	mkdir(t, served, "directory")
	write(t, served, "directory/file", "payload")
	mounted, _ := mount(t, served)
	if entries, err := mounted.List(t.Context(), "directory"); err != nil || len(entries) != 1 {
		t.Fatalf("warm listing: %+v,%v", entries, err)
	}
	before, err := served.elsewhere.Stat(t.Context(), "directory")
	if err != nil {
		t.Fatal(err)
	}
	holder := retainedSession(t, served.elsewhere)
	grant, err := holder.(storage.NodeReferences).OpenNodeRef(t.Context(), before.ID, storage.NodeRefOptions{Kind: storage.NodeDirectory, Target: storage.ChildCondition{State: storage.SameNode, NodeID: before.ID}, MetadataAccess: storage.ReadMetadata, Use: storage.UseClaim{Deny: storage.ReadEntries}})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
		if err := grant.Reference.Close(context.Background()); err != nil {
			t.Error(err)
		}
	})
	if entries, err := mounted.List(t.Context(), "directory"); !errors.Is(err, storage.ErrUseConflict) || entries != nil {
		t.Fatalf("cached listing bypassed authority claim: %+v,%v", entries, err)
	}
	bounded := listResult(t, 4096)
	if err := mounted.ListBounded(t.Context(), "directory", bounded); !errors.Is(err, storage.ErrUseConflict) {
		t.Fatalf("bounded listing bypassed authority claim: %v", err)
	}
	assertInvalidListResult(t, bounded)
	if err := grant.Reference.Close(t.Context()); err != nil {
		t.Fatal(err)
	}
	if entries, err := mounted.List(t.Context(), "directory"); err != nil || len(entries) != 1 {
		t.Fatalf("released listing: %+v,%v", entries, err)
	}
	bounded = listResult(t, 4096)
	if err := mounted.ListBounded(t.Context(), "directory", bounded); err != nil {
		t.Fatal(err)
	}
	if entries, err := bounded.Entries(); err != nil || len(entries) != 1 {
		t.Fatalf("released bounded listing: %+v,%v", entries, err)
	}
}
