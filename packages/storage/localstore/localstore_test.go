...
func TestSweepRetriesGarbageAfterBackgroundFailure(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if err := store.Write(t.Context(), "next-sweep", nil); err != nil {
			t.Fatal(err)
		}
		synctest.Wait()
		latest := store.MaintenanceStatus()
		if latest.LastSweepError != nil || latest.LastSweepRemoved != 0 {
			t.Fatalf("later empty background sweep = %+v, want the latest successful zero-removal result", latest)
		}
