...
// link's common times and opaque client metadata describe the link itself.
//
...
type Storage interface {
	...
	// above this interface depends on it being otherwise, because callers can apply separate common-time requests.
...
func (r *ListResult) Add(entry Entry) error {
	...
	attr := entry.Attr
	attr.Metadata = nil
	reservation, err := r.Reserve(int64(len(entry.Name)), int64(size), attr)
	if err != nil {
		return err
	}
...
// Reserve charges the raw name and canonical metadata envelope before either is loaded.
// Attr.Metadata must be absent until Commit; an empty envelope is six bytes. A database-backed implementation
...
func (r *ListResult) Reserve(nameBytes, metadataBytes int64, attr Attr) (*ListReservation, error) {
	...
	if len(attr.Metadata) != 0 {
		return nil, r.fail(syscall.EINVAL)
	}
...
// ReservePrefix charges one non-entry output prefix before any entry
// reservation. It does not create an Entry. Repeated, late, negative or
// oversized reservations invalidate the complete result.
...
func (r *ListReservation) Commit(name string, metadata map[string]OpaquePayload) error {
	...
	size, err := metadataSize(metadata)
	if err != nil {
		return r.result.fail(err)
	}
	if int64(size) != r.metadataBytes {
		return r.result.fail(fmt.Errorf("listing metadata length differs from reservation: %w", syscall.EIO))
	}
	r.attr.Metadata = CloneMetadata(metadata)
	r.committed = true
	r.result.pending--
...
type Attr struct {
	...
	Metadata   map[string]OpaquePayload
...
//
// A nil field is an attribute the caller is not changing. They are pointers because the
// zero time.Time is an explicit instant, not a sentinel for "leave this alone".
...
func (c AttrChange) Empty() bool {
	return c.BirthTime == nil && c.AccessTime == nil && c.ModTime == nil
}
...
func (c AttrChange) Check() error { return nil }
