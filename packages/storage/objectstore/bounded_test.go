...
func TestBoundedReadRefusesFromMetadataBeforeFetchingTheObject(t *testing.T) {
	...
	if err := volume.Write(t.Context(), "f", []byte("four")); err != nil {
		t.Fatal(err)
	}
	objects.gets = 0
	if got, err := volume.ReadBounded(t.Context(), "f", 3); got != nil || !errors.Is(err, syscall.EFBIG) {
	...
	if got, err := volume.ReadBounded(t.Context(), "f", 4); err != nil || string(got) != "four" {
...
func TestBoundedStorageCheckRefusesAnUnboundedDependency(t *testing.T) {
	...
	volume := objectstore.New(struct{ objectstore.Objects }{Objects: memory.New()}, meta)
	t.Cleanup(func() { _ = volume.Close() })
	if err := volume.CheckBounded(); err == nil {
	...
	volume2 := objectstore.New(memory.New(), struct{ metastore.Store }{Store: meta2})
	t.Cleanup(func() { _ = volume2.Close() })
	if err := volume2.CheckBounded(); err == nil {
...
func TestBoundedListDoesNotRetainTheEntryThatCrossesItsBudget(t *testing.T) {
	...
	for _, name := range []string{"a", "bb", "ccc"} {
		if err := volume.Create(t.Context(), name); err != nil {
			t.Fatal(err)
		}
	}
	...
	if err := volume.ListBounded(t.Context(), "", result); !errors.Is(err, syscall.EIO) {
