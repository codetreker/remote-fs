...
func TestObjectReferenceNameObservationRequiresNoMetadataPermission(t *testing.T) {
	...
	opened, err := references.OpenNodeRef(t.Context(), attr.ID, storage.NodeRefOptions{Action: objectAction(t), Kind: storage.NodeRegular,
	...
	command := storage.NameCommand{Action: objectAction(t), Kind: storage.NameRename, Name: storage.ChildName{Parent: storage.DirectoryTarget{NodeID: observed.ParentID}, RawLeaf: observed.RawLeaf},
