...
func objectObservationNode(t *testing.T, namespace storage.NamespaceAccess, kind storage.NameOperation, name storage.ChildName, metadata map[string][]byte) storage.Attr {
	t.Helper()
	created, err := namespace.MutateName(t.Context(), storage.NameCommand{Kind: kind, Name: name, Target: storage.ChildCondition{State: storage.Absent}, Initial: storage.InitialFields{Metadata: metadata}})
...
func TestObjectDirectoryMetadataObservationKeepsApplicationPermissionsSeparate(t *testing.T) {
	...
	opened, err := objectCapability[storage.NodeReferences](t, session).OpenNodeRef(t.Context(), directory.ID, storage.NodeRefOptions{
		Kind: storage.NodeDirectory, Target: storage.ChildCondition{State: storage.SameNode, NodeID: directory.ID}, Use: storage.UseClaim{Deny: storage.ReadEntries},
	})
...
func TestObjectDirectoryMetadataObservationPreservesGuardsScopesAndCancellation(t *testing.T) {
	...
	opened, err := objectCapability[storage.NodeReferences](t, session).OpenNodeRef(t.Context(), directory.ID, storage.NodeRefOptions{Kind: storage.NodeDirectory, Target: storage.ChildCondition{State: storage.SameNode, NodeID: directory.ID}})
	if err != nil || opened.Reference == nil {
