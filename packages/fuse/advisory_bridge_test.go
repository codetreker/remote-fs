...
func TestAdvisoryOwnerCannotBorrowAnotherNodeIdentity(t *testing.T) {
	v := advisoryVolume(t)
	h := advisoryHandle(t, v, true, true)
	h.node.id.node++
	lk := gofuse.FileLock{Start: 0, End: math.MaxInt64, Typ: syscall.F_WRLCK}
	if errno := h.Setlk(t.Context(), 1, &lk, gofuse.FUSE_LK_FLOCK); errno != syscall.ESTALE {
		t.Fatalf("scope bound to another node returned %v, want ESTALE", errno)
	}
	if len(v.lockOwners) != 0 || len(v.lockGroups) != 0 {
		t.Fatal("rejected scope retained local owner state")
	}
}
...
func TestAdvisoryReleaseClosesReferenceAfterOwnerCleanupFailure(t *testing.T) {
	...
	close(cancelled)
	forwarded, done, ok := v.raw.begin(cancelled, rawRequest{kind: rawRelease, owner: 7})
	if !ok {
