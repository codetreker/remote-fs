...
func (v *volume) cleanupContext(ctx context.Context) (context.Context, context.CancelFunc) {
	deadline := time.Now().Add(v.flushTimeout)
...
func (v *volume) closeError(err error) error {
	if err == nil {
		return nil
	}
