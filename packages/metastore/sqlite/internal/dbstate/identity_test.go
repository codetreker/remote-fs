		...
		for _, commit := range []bool{false, true} {
			t.Run(kind+map[bool]string{false: " rollback", true: " commit"}[commit], func(t *testing.T) {
				...
				if kind == "node" {
					...
					execState(t, tx, `INSERT INTO nodes (id, volume, kind, size, atime_sec, atime_nsec, mtime_sec, mtime_nsec, directory_revision)
						SELECT ?, volume, kind, size, atime_sec, atime_nsec, mtime_sec, mtime_nsec, directory_revision FROM nodes WHERE id=1`, next)
				} else {
