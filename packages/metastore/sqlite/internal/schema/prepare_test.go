...
func testFile(t *testing.T, db *sql.DB, volume, parent int64, name string, size int64, detached bool) (int64, string) {
	...
	if !detached {
		entry, err := dbstate.AllocateEntryID(t.Context(), tx)
		if err != nil {
			t.Fatal(err)
		}
		execute(t, tx, `INSERT INTO entries (id, volume, parent, name, node) VALUES (?, ?, ?, ?, ?)`, entry, volume, parent, []byte(name), id)
	}
...
func testChange(t *testing.T, db *sql.DB, volume, parent int64, name string) {
	t.Helper()
	tx := testTransaction(t, db)
	subject, err := dbstate.AllocateNodeID(t.Context(), tx)
	if err != nil {
		t.Fatal(err)
	}
	entry, err := dbstate.AllocateEntryID(t.Context(), tx)
	...
	var root int64
	if err := tx.QueryRow(`SELECT root FROM volumes WHERE id=?`, volume).Scan(&root); err != nil {
		t.Fatal(err)
	}
	before := &metastore.EventImage{
		Attr: storage.Attr{ID: uint64(subject), Kind: storage.NodeRegular, MetadataRevision: 1, AccessTime: time.Unix(0, 0).UTC(), ModTime: time.Unix(0, 0).UTC()},
		Location: storage.EntryLocation{State: storage.LocationLinked, RootNodeID: uint64(root), NodeID: uint64(subject),
			Ancestors: []storage.EntryCondition{{ParentID: uint64(parent), DirectoryRevision: 1, EntryID: storage.EntryID(entry), NodeID: uint64(subject), Name: []byte(name)}}},
	}
	if err := changes.Record(t.Context(), tx, volume, metastore.Change{
		Notification: &metastore.Notification{SubjectID: subject, SubjectKind: storage.NodeRegular, ChangeMask: metastore.ChangeName, Before: before},
...
func TestPreparePreservesVolumeAndAdvancesDurableState(t *testing.T) {
	...
	if kind != int64(storage.NodeDirectory) || size != 0 || used != 0 || incarnation == "" {
