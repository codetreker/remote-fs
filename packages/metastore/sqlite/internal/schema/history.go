...
func validateLogIntegrityVersion(ctx context.Context, db sqlvalue.Queryer, volume *int64, version int, predecessors bool) error {
	...
	if version >= firstClientCapabilitySchemaVersion {
		...
		c.change_sec IS NOT NULL OR c.change_nsec IS NOT NULL OR c.metadata IS NOT NULL OR
		c.link_target IS NOT NULL OR c.directory_revision IS NOT NULL`
		requiredExtra = ` OR c.metadata IS NULL OR c.link_target IS NULL`
		...
		(c.node_kind=3 AND (c.size<=0 OR c.content IS NOT NULL OR length(c.link_target)!=c.size)) OR
		(c.node_kind!=3 AND length(c.link_target)!=0) OR
		(c.node_kind!=2 AND length(c.directory_revision)!=0) OR length(c.directory_revision)>64 OR
	...
	if version < firstClientCapabilitySchemaVersion {
		changeArgs = append(changeArgs, int64(math.MaxUint32), int64(fs.ModeType), int64(fs.ModeDir), int64(fs.ModeType), int64(fs.ModeDir), int64(fs.ModeType))
	}
	if err := db.QueryRowContext(ctx, `
		SELECT count(*)
		FROM changes c
