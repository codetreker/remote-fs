...
func validateIntegrityBytes(
	...
	for rows.Next() {
		...
		remaining -= length.Int64
		if version >= firstDeleteIntentOwnerSchemaVersion {
			if ownerClass != "text" || !ownerLength.Valid || ownerLength.Int64 < 1 || ownerLength.Int64 > storage.MaxDeleteIntentOwnerBytes {
				rows.Close()
				return fmt.Errorf("a deletion-intent owner has an invalid representation: %w", syscall.EIO)
			}
			if ownerLength.Int64 > remaining {
				rows.Close()
				return fmt.Errorf("entry, change, and deletion-intent fields exceed the %d-byte integrity limit: %w", maxIntegrityBytes, syscall.EFBIG)
			}
