...
func validateNeutralNodeValues(ctx context.Context, db sqlvalue.Queryer, volume *int64) error {
	where := ""
	var args []any
	if volume != nil {
		where = "volume=? AND "
...
func validateNeutralNodeRelationships(ctx context.Context, db sqlvalue.Queryer, volume *int64) error {
