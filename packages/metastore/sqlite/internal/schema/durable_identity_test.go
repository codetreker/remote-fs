...
func TestDeleteIntentNamesConsumeIntegrityByteBudgetBeforeInspection(t *testing.T) {
	...
	execute(t, db, `INSERT INTO volumes(id,name,root,used,delete_intent_high_water) VALUES(1,'v',1,0,1)`)
	...
	execute(t, db, `INSERT INTO delete_intents
		...
		VALUES('0123456789abcdef0123456789abcdef',1,'o',1,1,1,X'6e616d65',zeroblob(16),zeroblob(32),1,?,NULL,0,0)`,
		storage.DeleteIntentArmed)
	volume := int64(1)
	if err := validateIntegrityBytes(t.Context(), db, &volume, 5, schema.Version()); err != nil {
		t.Fatalf("exact deletion-intent name budget = %v", err)
	}
	if err := validateIntegrityBytes(t.Context(), db, &volume, 4, schema.Version()); !errors.Is(err, syscall.EFBIG) {
...
func TestDeleteIntentIntegrityRejectsUnknownCleanupErrno(t *testing.T) {
	...
	execute(t, db, `INSERT INTO delete_intents
		(intent,volume,owner,sequence,node,parent,name,reference,request_hash,if_empty,outcome,failure,updated_sec,updated_nsec)
		VALUES('0123456789abcdef0123456789abcdef',1,'o',1,1,1,X'6e616d65',zeroblob(16),zeroblob(32),1,?,999,0,0)`,
