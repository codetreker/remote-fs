...
func TestAVolumeOutlivesTheStoreThatMadeIt(t *testing.T) {
	...
	if err := first.SetAttr(t.Context(), "d/f", storage.AttrChange{ModTime: &changed}); err != nil {
		t.Fatal(err)
	}
	if err := first.Close(); err != nil {
	...
	if node.Kind != storage.NodeRegular || !node.ModTime.Equal(changed) {
		t.Fatalf("the file has kind %v and modification time %v, want regular file at %v",
