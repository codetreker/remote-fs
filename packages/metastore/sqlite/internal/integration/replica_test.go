...
func requireSame(t *testing.T, from *sqlite.Store, into *sqlite.Replica) {
	...
	for at, node := range want {
		...
		if mirrored.ID != node.ID || mirrored.Kind != node.Kind || mirrored.Size != node.Size ||
			!mirrored.ModTime.Equal(node.ModTime) || !mirrored.AccessTime.Equal(node.AccessTime) || !sameInstant(mirrored.BirthTime, node.BirthTime) || !sameInstant(mirrored.ChangeTime, node.ChangeTime) || !sameOpaqueMetadata(mirrored.Metadata, node.Metadata) || !bytes.Equal(mirrored.LinkTarget, node.LinkTarget) || !bytes.Equal(mirrored.DirectoryRevision, node.DirectoryRevision) {
