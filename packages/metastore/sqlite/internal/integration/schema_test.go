...
func writePreRetainedDatabase(t *testing.T, version int) string {
	...
	if version < 3 || version > 5 {
		t.Fatalf("unsupported historical schema version %d", version)
	}
	for next := 4; next <= version; next++ {
		name := map[int]string{4: "0004_lease_recovery.sql", 5: "0005_retained_files.sql"}[next]
		migration, err := os.ReadFile(filepath.Join("..", "schema", "migrations", name))
		if err != nil {
			t.Fatal(err)
		}
		damageDatabase(t, path, string(migration))
		damageDatabase(t, path, fmt.Sprintf(`UPDATE schema_version SET version = %d`, next))
	}
	return path
}
...
func TestClientMigrationWitnessFailurePreservesCommittedHistoryForRecovery(t *testing.T) {
	path := writePreRetainedDatabase(t, 5)
	before := historicalLeaseRows(t, path)
	failure := errors.New("migration witness publication unavailable")
	witness := &recordingWitness{database: path, acceptErr: failure}
	store, err := sqlite.OpenBoundDurableWithOptions(t.Context(), path, "A", durableStoreID, 1024,
		sqlite.DefaultOptions(), sqlite.RequireExistingVolume, sqlite.DurableStartup{
			Accepted: historicalLeaseDurableState, CheckpointedGeneration: historicalLeaseDurableState.Generation,
		}, witness)
	if store != nil {
		store.Abort()
		t.Fatal("unaccepted migration exposed a Store")
	}
	...
	accepted, visible := witness.accepts()
	if len(accepted) != 1 || len(visible) != 1 || accepted[0] != want || visible[0] != want {
		t.Fatalf("migration must commit before witness publication: accepted=%+v visible=%+v want=%+v", accepted, visible, want)
	}
	wal, err := os.Stat(path + "-wal")
	if err != nil || wal.Size() == 0 {
		t.Fatalf("unaccepted migration lost recovery WAL: %v, %v", wal, err)
	}
	recoveryWitness := &recordingWitness{database: path}
	...
	if err != nil {
		t.Fatalf("recover committed migration: %v", err)
	}
	t.Cleanup(func() {
	...
	assertHistoricalLeaseRows(t, path, before)
	want.Generation++
	accepted, visible = recoveryWitness.accepts()
	if len(accepted) != 1 || len(visible) != 1 || accepted[0] != want || visible[0] != want {
		t.Fatalf("recovery acceptance=%+v visible=%+v want=%+v", accepted, visible, want)
	}
}

func TestClientMigrationMetadataBudgetRefusalPreservesEveryVolume(t *testing.T) {
	path := writePreRetainedDatabase(t, 5)
	options := sqlite.DefaultOptions()
	// Each volume contains two nodes and two retained attribute images, each
	// with the 46-byte canonical historical permissions envelope.
	options.MaxMetadataBytes = 4*46 - 1
	assertRetainedMigrationRefused(t, path, 5, options, syscall.EFBIG)
	...
	if err != nil {
		t.Fatalf("exact per-volume metadata budget: %v", err)
	}
	t.Cleanup(func() {
		if err := store.Close(); err != nil {
			t.Error(err)
	...
	for _, volume := range []int{1, 2} {
		var used int64
		if err := db.QueryRow(`SELECT metadata_used FROM volumes WHERE id=?`, volume).Scan(&used); err != nil || used != options.MaxMetadataBytes {
			t.Fatalf("volume %d metadata charge=%d, error=%v; want %d", volume, used, err, options.MaxMetadataBytes)
		}
	}
}
...
func TestAVersionOneDatabaseIsCarriedForwardIntact(t *testing.T) {
	...
	if historicalNodeMode(t, file).Perm() != 0o640 {
		t.Fatalf("the file came back with mode %v, want 0640", historicalNodeMode(t, file))
	}
