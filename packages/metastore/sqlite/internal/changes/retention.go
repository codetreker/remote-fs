...
// Writes and volume preparation invoke it; an idle log has no background trimmer.
func Trim(ctx context.Context, tx *sql.Tx, volume int64, window Window) error {
	oldest, oldestAt, err := oldestEntry(ctx, tx, volume)
	if err != nil || oldest == 0 {
		return err
	}
	newest, err := newestEntry(ctx, tx, volume)
	if err != nil {
		return err
	}
	...
	// shared with the other volumes in this database — an overstatement only ever costs the
	...
	if err := tx.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM changes WHERE volume = ?`, volume).Scan(&held); err != nil {
		return err
	}
	...
	byVolume, err := nthOldest(ctx, tx, volume, held-int64(window.Cap))
	if err != nil {
		return err
	}
	byFloor, err := nthOldest(ctx, tx, volume, held-int64(window.Floor))
	if err != nil {
		return err
	}
	byAge, err := lastBefore(ctx, tx, volume, time.Now().Add(-window.Age))
	if err != nil {
		return err
	}
	...
	if _, err := tx.ExecContext(ctx,
		`DELETE FROM changes WHERE volume = ? AND position <= ?`, volume, cut); err != nil {
		return err
	}
	...
	// instead would be working it out from a distance that the other volumes in this
	// database set, which is a fact about them rather than about this replica.
	//
	// What makes it usable is that it is a position this volume actually held and no longer
	...
	// volume changes faster than the log was configured to hold. When both would have cut
	// to the same place, volume did it — the entries were over the cap whether or not anybody
	// had been away.
	_, err = tx.ExecContext(ctx,
		`UPDATE logs SET trimmed_through = ?, trimmed_by_age = ? WHERE volume = ?`,
		int64(cut), byAge > byVolume, volume)
	return err
}

// oldestEntry reports the oldest position still held and the instant it was recorded, or zero
// for a log holding nothing.
func oldestEntry(ctx context.Context, tx *sql.Tx, volume int64) (metastore.Position, time.Time, error) {
	var position, sec, nsec int64
	switch err := tx.QueryRowContext(ctx,
		`SELECT position, recorded_sec, recorded_nsec FROM changes
		 WHERE volume = ? ORDER BY position LIMIT 1`, volume).Scan(&position, &sec, &nsec); {
...
// entry at it.
func newestEntry(ctx context.Context, tx *sql.Tx, volume int64) (metastore.Position, error) {
	var newest int64
	err := tx.QueryRowContext(ctx,
		`SELECT COALESCE(MAX(position), 0) FROM changes WHERE volume = ?`, volume).Scan(&newest)
...
func nthOldest(ctx context.Context, tx *sql.Tx, volume, n int64) (metastore.Position, error) {
	if n <= 0 {
		return 0, nil
	}
	var position int64
	err := tx.QueryRowContext(ctx,
		`SELECT position FROM changes WHERE volume = ? ORDER BY position LIMIT 1 OFFSET ?`,
		volume, n-1).Scan(&position)
	if err != nil {
		return 0, err
	}
...
// as a volume's entries are recorded in the order their positions were allocated. A wall
...
func lastBefore(ctx context.Context, tx *sql.Tx, volume int64, cutoff time.Time) (metastore.Position, error) {
	...
	err := tx.QueryRowContext(ctx, `
		...
		volume, sec, sec, nsec).Scan(&position)
