...
func (s *Store) openAtomicChild(ctx context.Context, name storage.ChildName, kind storage.NodeKind, read, write bool,
	...
	inspect := func(tx *sql.Tx) error {
		...
		if err := s.checkNamespaceGuards(ctx, tx, guards); err != nil {
			return err
		}
		if closeIntent != nil {
			if err := s.checkNamespaceGuards(ctx, tx, closeIntent.Guards); err != nil {
				return err
			}
		}
	...
	if !found && kind == storage.NodeSymlink {
		intent.added = int64(len(initial.OnCreate.LinkTarget))
	}
	if found {
		intent.nodes = nil
	...
	return s.finishReferenceOpen(ctx, file, claimed, closeIntent != nil, state, outcome, err)
}

func (s *Store) finishReferenceOpen(ctx context.Context, file *retainedFile, claimed, closeIntent bool, state metastore.FileState, outcome storage.OpenOutcome, err error) (*retainedFile, metastore.FileState, storage.OpenOutcome, error) {
	if err != nil {
