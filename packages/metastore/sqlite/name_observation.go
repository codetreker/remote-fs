...
func (s *Store) nameObservationHeader(ctx context.Context, tx *sql.Tx, id int64) (storage.NameObservation, int64, error) {
	...
	rows, err := tx.QueryContext(ctx, `SELECT
		CASE WHEN typeof(e.volume)='integer' THEN e.volume ELSE 0 END,
	...
	count := 0
	for rows.Next() {
		var entryVolume, parentKind, parentVolume, parentDetached int64
		if err := rows.Scan(&entryVolume, &parent, &nameBytes, &parentKind, &parentVolume, &parentDetached); err != nil {
			rows.Close()
			return storage.NameObservation{}, 0, err
		}
		count++
		if count > 1 || entryVolume != s.volume || parent < 1 || parent == id || parentKind != int64(storage.NodeDirectory) || parentVolume != s.volume || parentDetached != 0 || nameBytes < 1 {
			rows.Close()
			return storage.NameObservation{}, 0, syscall.EIO
		}
	}
