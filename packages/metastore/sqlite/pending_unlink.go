...
func (s *Store) armCloseIntent(ctx context.Context, tx *sql.Tx, f *retainedFile, intent *storage.CloseIntent) error {
	...
	_, err = tx.ExecContext(ctx, `INSERT INTO delete_intents
		...
		VALUES(?,?,?,?,?,?,?,?,?,?,?,NULL,?,?)`, string(intent.ID), s.volume, string(intent.Owner), sequence, f.id, parent, name, reference,
...
func (s *Store) QueryDeleteIntent(ctx context.Context, owner storage.DeleteIntentOwner, id storage.DeleteIntentID) (storage.DeleteIntentStatus, error) {
	...
	err := s.inspect(ctx, func(tx *sql.Tx) error {
		...
		if err := tx.QueryRowContext(ctx, `SELECT node,outcome,failure FROM delete_intents WHERE volume=? AND owner=? AND intent=?`,
			s.volume, string(owner), string(id)).Scan(&node, &status.Outcome, &failure); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) ListDeleteIntents(ctx context.Context, owner storage.DeleteIntentOwner, after storage.DeleteIntentCursor, limit int) (storage.DeleteIntentPage, error) {
	if err := owner.Check(); err != nil {
	...
	if err != nil {
		return storage.DeleteIntentPage{}, sqlerr.Failure(err)
	}
	return page, nil
}
...
func (s *Store) AcknowledgeDeleteIntent(ctx context.Context, command storage.AcknowledgeDeleteIntentCommand) error {
	...
	err := s.mutateTransactionLocked(ctx, ctx, nil, func(tx *sql.Tx) error {
		...
		err := tx.QueryRowContext(ctx, `SELECT outcome FROM delete_intents WHERE volume=? AND owner=? AND intent=?`, s.volume, string(command.Owner), string(command.Intent)).Scan(&outcome)
		if errors.Is(err, sql.ErrNoRows) {
			return nil
		}
		if err != nil {
		...
		result, err := tx.ExecContext(ctx, `DELETE FROM delete_intents WHERE volume=? AND owner=? AND intent=?`, s.volume, string(command.Owner), string(command.Intent))
		if err != nil {
			return err
		}
