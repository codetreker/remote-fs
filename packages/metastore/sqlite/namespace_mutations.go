...
func (s *Store) checkPendingNameTarget(ctx context.Context, tx *sql.Tx, node int64, uses []storage.TargetUse) error {
	pending, err := s.nodePendingUnlink(ctx, tx, node)
	if err != nil || !pending {
		return err
	}
...
func (s *Store) prepareNamespaceMutation(ctx context.Context, tx *sql.Tx, command storage.NameCommand) (namespaceMutation, error) {
	...
	if pending, err := s.nodePendingUnlink(ctx, tx, mutation.parent.ID); err != nil {
		return mutation, err
	} else if pending {
		return mutation, storage.ErrPendingDelete
	}
	var found bool
	...
	if found {
		if err := s.checkPendingNameTarget(ctx, tx, mutation.source.ID, command.Uses); err != nil {
			return mutation, err
		}
	}
	switch command.Kind {
...
func (s *Store) prepareNamespaceRename(ctx context.Context, tx *sql.Tx, command storage.NameCommand, mutation namespaceMutation) (namespaceMutation, error) {
	...
	if pending, err := s.nodePendingUnlink(ctx, tx, mutation.destination.ID); err != nil {
		return mutation, err
	} else if pending {
		return mutation, storage.ErrPendingDelete
	}
	...
	if occupied && mutation.displaced.ID != mutation.source.ID {
		if err := s.checkPendingNameTarget(ctx, tx, mutation.displaced.ID, command.Uses); err != nil {
			return mutation, err
		}
	}
