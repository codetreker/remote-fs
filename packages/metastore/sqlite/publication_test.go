...
func TestSQLitePublicationRejectsAnonymousMutationOfGrantedFiles(t *testing.T) {
	for _, mode := range []locking.Mode{locking.Shared, locking.Exclusive} {
		...
		for _, operation := range []string{"commit", "metadata", "access time", "modification time", "empty attributes", "remove", "rename source", "rename destination", "self rename"} {
			t.Run(string(mode)+"/"+operation, func(t *testing.T) {
				...
				case "metadata":
					node := f.node(t, "locked")
					_, err = f.store.SetMetadata(t.Context(), uint64(node.ID), "test.state", nil, []byte{0, 255, 6})
				case "access time":
...
func TestSQLitePublicationAuthorizesExplicitAttributes(t *testing.T) {
	...
	owner := f.owner(t)
	grant := f.grant(t, owner, "file", locking.Exclusive)
	birth := time.Unix(1_700_000_000, 5)
	...
	node := f.node(t, "file")
	metadata, err := f.store.SetMetadata(publicationScope(t.Context(), owner, grant), uint64(node.ID), "test.state", nil, []byte{0, 255, 6})
	if err != nil {
		t.Fatal(err)
	}
	node = f.node(t, "file")
	if node.BirthTime == nil || !node.BirthTime.Equal(birth) || !node.AccessTime.Equal(atime) || !node.ModTime.Equal(mtime) || !bytes.Equal(node.Metadata["test.state"].Data, []byte{0, 255, 6}) || len(metadata.Version) == 0 || !bytes.Equal(node.Metadata["test.state"].Version, metadata.Version) {
...
func TestSQLiteStagedCommitChecksTheActualPublicationTarget(t *testing.T) {
	...
	f.put(t, t.Context(), "file", 3)
	object := f.stage(t, t.Context(), "file", 7)
	originalID := f.node(t, "file").ID
	if err := f.store.Rename(t.Context(), "file", "moved"); err != nil {
		t.Fatal(err)
	}
	moved := f.node(t, "moved")
	if moved.ID != originalID {
		t.Fatal("rename changed the staged target identity")
	}
	f.put(t, t.Context(), "file", 5)
	owner := f.owner(t)
	grant := f.grant(t, owner, "file", locking.Exclusive)
	...
	requirePublicationCode(t, f.store.Commit(t.Context(), "file", object), locking.Conflict)
	if !reflect.DeepEqual(f.node(t, "file"), replacement) || !reflect.DeepEqual(f.node(t, "moved"), moved) {
	...
	if !reflect.DeepEqual(f.node(t, "moved"), moved) {
