...
func (s *Store) resolveReturnedNode(ctx context.Context, tx *sql.Tx, path string) (metastore.Node, error) {
	if !storage.HasAttrResultBudget(ctx) {
		return s.resolve(ctx, tx, path)
	}
...
func (s *Store) resolveNodeID(ctx context.Context, tx *sql.Tx, path string) (int64, bool, error) {
	...
	var id int64
	err = tx.QueryRowContext(ctx, `SELECT node FROM entries WHERE volume=? AND parent=? AND name=?`, s.volume, parent.ID, name).Scan(&id)
	if errors.Is(err, sql.ErrNoRows) {
		return 0, false, nil
	}
	if err != nil {
