...
// The header carries only scalars, bounded observation tokens and payload lengths.
// Listings and snapshots reserve their representation before loading payloads.
var nodeHeaderColumns = fmt.Sprintf(`
	...
	storage.MaxObservationTokenBytes, storage.MaxObservationTokenBytes)

var nodeColumns = nodeHeaderColumns + fmt.Sprintf(`, CASE WHEN typeof(n.content)='text' THEN n.content END,
	CASE WHEN typeof(n.metadata)='blob' AND length(n.metadata)<=%d THEN n.metadata END,
	CASE WHEN typeof(n.link_target)='blob' AND length(n.link_target)<=%d THEN n.link_target END`,
	storage.MaxMetadataBytes, storage.MaxLinkTargetBytes)
...
type nodeHeader struct {
	...
	contentBytes, metadataBytes int64
	targetBytes, valid          int64
}
...
func (s *nodeHeader) node() (metastore.Node, error) {
	...
	if s.metadataBytes > storage.MaxMetadataBytes || s.targetBytes > storage.MaxLinkTargetBytes {
		return metastore.Node{}, fmt.Errorf("stored node payload exceeds its bound: %w", syscall.EFBIG)
	}
	kind := storage.NodeKind(s.kind)
	// Metadata-only replicas can retain historical unknown directory tokens. The
	// authority's schema validation and mutation helpers require its current token.
	if kind == storage.NodeDirectory && (s.size != 0 || s.contentBytes != 0) ||
		...
		return metastore.Node{}, fmt.Errorf("stored node kind and payload disagree: %w", syscall.EIO)
	}
