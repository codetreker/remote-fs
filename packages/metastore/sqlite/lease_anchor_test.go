...
func leaseAnchorFixture(t *testing.T) LeaseAnchorConfig {
	t.Helper()
	root := t.TempDir()
	...
	return LeaseAnchorConfig{
		Directory: root, Name: ".leases", Identity: "workspace/objects",
		BindingFD: fd, RecoveryStart: time.Now(), Initialize: true,
	}
}

func mustLeaseAnchor(t *testing.T, config LeaseAnchorConfig) *LeaseAnchor {
	t.Helper()
	a, err := OpenLeaseAnchor(config)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
...
func anchorEvidence(a *LeaseAnchor) LeaseEvidence {
	...
	for _, name := range []string{"intent", "witness", "binding", "identity", "state-slot", "state-location"} {
		t.Run(name, func(t *testing.T) {
			...
			for _, initialize := range []bool{false, true} {
				...
				if opened, err := OpenLeaseAnchor(config); !errors.Is(err, syscall.EIO) {
					if opened != nil {
						_ = opened.Close()
					}
...
func TestLeaseAnchorRecoversOnlyMatchingInitialization(t *testing.T) {
	...
	config := leaseAnchorFixture(t)
	config.Initialize = false
	if a, err := OpenLeaseAnchor(config); !errors.Is(err, syscall.EIO) {
		if a != nil {
			_ = a.Close()
		}
...
func TestLeaseAnchorRejectsRegressionsAndIdentityChanges(t *testing.T) {
	...
	for name, change := range map[string]func(*LeaseEvidence){
		"decrease":   func(e *LeaseEvidence) { e.MaxLease-- },
		"skip":       func(e *LeaseEvidence) { e.Generation++ },
		"replay":     func(e *LeaseEvidence) { e.Generation = 1 },
		"database":   func(e *LeaseEvidence) { e.DatabaseID = strings.Repeat("b", 32) },
		"state":      func(e *LeaseEvidence) { e.StateID = strings.Repeat("c", 32) },
		"bad-id":     func(e *LeaseEvidence) { e.DatabaseID = "corrupt" },
		"negative":   func(e *LeaseEvidence) { e.MaxLease = -1 },
		"generation": func(e *LeaseEvidence) { e.Generation = -1 },
	} {
		t.Run(name, func(t *testing.T) {
			next := accepted
			next.Generation++
...
func TestLeaseAnchorChecksConfigurationAndRecordEncoding(t *testing.T) {
	config := leaseAnchorFixture(t)
	for name, change := range map[string]func(*LeaseAnchorConfig){
		"relative": func(c *LeaseAnchorConfig) { c.Directory = "relative" },
		"name":     func(c *LeaseAnchorConfig) { c.Name = "../elsewhere" },
		"identity": func(c *LeaseAnchorConfig) { c.Identity = "" },
		"fd":       func(c *LeaseAnchorConfig) { c.BindingFD = -1 },
		"time":     func(c *LeaseAnchorConfig) { c.RecoveryStart = time.Time{} },
	} {
		t.Run(name, func(t *testing.T) {
			invalid := config
			change(&invalid)
			if a, err := OpenLeaseAnchor(invalid); !errors.Is(err, syscall.EINVAL) {
				if a != nil {
					_ = a.Close()
				}
	...
	if err := decodeLeaseRecord(encoded, "witness", &LeaseEvidence{}); !errors.Is(err, syscall.EIO) {
...
func TestLeaseAnchorRejectsRemoteFilesystemsBeforeMutation(t *testing.T) {
	for _, filesystem := range []int64{
		...
		t.Run(fmt.Sprintf("%x", filesystem), func(t *testing.T) {
			...
			if a, err := openLeaseAnchor(config, operations); !errors.Is(err, syscall.EOPNOTSUPP) {
				if a != nil {
					_ = a.Close()
				}
	...
	for _, reopen := range []bool{false, true} {
		for name, change := range map[string]func(*leaseAnchorOperations){
			...
			t.Run(fmt.Sprintf("reopen=%v/%s", reopen, name), func(t *testing.T) {
				...
				operations := leaseAnchorSystemOperations
				change(&operations)
				if a, err := openLeaseAnchor(config, operations); !errors.Is(err, injected) {
					if a != nil {
						_ = a.Close()
					}
...
func assertLeaseProbesAbsent(t *testing.T, config LeaseAnchorConfig) {
	t.Helper()
