...
func validateChangeMeta(meta Change, lengths ChangePayloadLengths) error {
	...
	if len(meta.Name) != 0 || (meta.From != nil && len(meta.From.Name) != 0) || (meta.Node != nil && (len(meta.Node.Content) != 0 || len(meta.Node.Metadata) != 0 || len(meta.Node.LinkTarget) != 0)) {
...
func (r *ChangeReservation) Commit(name, fromName []byte, content Key, metadata, target []byte) error {
	...
	if int64(len(name)) != r.lengths.Name || int64(len(fromName)) != r.lengths.FromName || int64(len(content)) != r.lengths.Content || int64(len(metadata)) != r.lengths.Metadata || int64(len(target)) != r.lengths.Target {
