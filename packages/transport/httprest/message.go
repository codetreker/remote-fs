...
// AttrChange omits exactly the attributes the caller leaves unchanged.
...
func AttrChangeOf(c storage.AttrChange) *AttrChange {
	return &AttrChange{AccessTime: optionalTimeOf(c.AccessTime), ModTime: optionalTimeOf(c.ModTime),
		BirthTime: optionalTimeOf(c.BirthTime), ChangeTime: optionalTimeOf(c.ChangeTime)}
}

func (c AttrChange) Storage() storage.AttrChange {
	return storage.AttrChange{AccessTime: optionalTimeStorage(c.AccessTime), ModTime: optionalTimeStorage(c.ModTime),
		BirthTime: optionalTimeStorage(c.BirthTime), ChangeTime: optionalTimeStorage(c.ChangeTime)}
}
