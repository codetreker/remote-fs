...
func TestRequestURLKeepsTheBasePrefix(t *testing.T) {
	for _, raw := range []string{"http://example.invalid/under/here", "http://example.invalid/under/here/"} {
		...
		if want := "/under/here/v1/stat"; u.Path != want {
...
func TestParseRequestRejectsMalformedRequests(t *testing.T) {
	cases := []struct {
		...
		{"a query that does not parse", http.MethodGet, "/v1/stat?path=%zz", httprest.ErrOperands},
		{"no path at all", http.MethodGet, "/v1/stat", httprest.ErrOperands},
		{"an empty query", http.MethodGet, "/v1/stat?", httprest.ErrOperands},
		{"the path given twice", http.MethodGet, "/v1/stat?path=a&path=b", httprest.ErrOperands},
...
func TestParseRequestAcceptsTheRoot(t *testing.T) {
	u, err := url.ParseRequestURI("/v1/list?path=")
	if err != nil {
		t.Fatal(err)
	}
	got, err := httprest.ParseRequest(http.MethodGet, u)
...
func ExampleRequest_URL() {
	...
	fmt.Println(u)
	// Output: http://server.example:8080/v1/read?path=notes%2Fone+two.txt
}
