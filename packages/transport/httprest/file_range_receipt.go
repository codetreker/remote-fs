...
func rangeResponseBound(commands []storage.RangeCommand) int64 {
	...
	command := storage.RangeCommand{Domain: 255, Mode: 255, Range: storage.Range{Kind: 255, Start: math.MaxUint64, Length: math.MaxUint64, CutAt: math.MaxUint64}, Edit: 255, Claim: claim, Wait: false, Conversion: 255, Policy: storage.RangePolicy{DenySelf: 255, DenyOthers: 255}}
	effects := len(commands)
	if len(commands) > 0 && commands[0].Conversion == storage.DropBeforeAcquire {
		effects = storage.MaxRangeEffects
	}
	attempt := storage.RangeAttempt{Request: request, State: storage.Granted, Commands: commands, Conflict: storage.RangeConflict{Found: false, Owner: storage.OwnerDiagnostic(math.MaxUint64), Range: command.Range, Mode: 255}, Rejection: "unsupported", EverGranted: false, HistoryRemaining: time.Duration(math.MaxInt64)}
	...
	for range effects {
		attempt.Effects = append(attempt.Effects, storage.RangeEffect{Claim: claim, Command: command, Released: false})
	}
	response := fileResponse{Epoch: math.MaxUint64, Data: []byte{}, Attempt: &attempt}
