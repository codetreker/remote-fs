...
func describe(r metastore.Row) string {
	return fmt.Sprintf("parent=%d name=%q id=%d kind=%v size=%d accessed=%d.%09d changed=%d.%09d content=%q birth=%v change=%v metadata=%v link=%q directory_revision=%q",
		...
		wireOptionalInstant(r.Node.BirthTime), wireOptionalInstant(r.Node.ChangeTime), r.Node.Metadata, r.Node.LinkTarget, r.Node.DirectoryRevision)
}
