...
func TestLostRangeApplyResponseIsReconciledByQuery(t *testing.T) {
	ctx := context.Background()
	client, _, backend := retainedHTTPFixture(t, DefaultFileLimits())
	if err := backend.Write(ctx, "file", []byte("data")); err != nil {
		t.Fatal(err)
	...
	session, file := openRetainedFixture(t, client)
	ranges, owners := rangeControlFixture(t, session, file, 1)
	request, err := storage.NewLockRequestID(session.epoch)
	if err != nil {
		t.Fatal(err)
	}
	command := storage.RangeCommand{Domain: storage.DomainWholeFile, Edit: storage.Replace, Mode: storage.RangeExclusive, Range: storage.Range{Kind: storage.Bytes, Length: uint64(math.MaxInt64) + 1}}
	original := client.http.Transport
	...
	client.http.Transport = fileRoundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		response, callErr := original.RoundTrip(req)
		if callErr == nil && envelope.Op == storage.OpFileRangeApply && lost.CompareAndSwap(false, true) {
			_, _ = io.Copy(io.Discard, response.Body)
			_ = response.Body.Close()
			return nil, errors.New("lost range receipt")
		}
		return response, callErr
	})
	if result, err := ranges.Apply(ctx, owners[0], []storage.RangeCommand{command}, request); storage.ErrnoOf(err) != syscall.EIO || !reflect.DeepEqual(result, storage.RangeAttempt{}) {
		t.Fatalf("lost apply returned %+v, %v", result, err)
	}
	result, err := ranges.Query(ctx, owners[0], request)
	if err != nil || result.State != storage.Granted || result.Request != request || !result.EverGranted {
		t.Fatalf("query did not recover the original receipt: %+v, %v", result, err)
	}
}
