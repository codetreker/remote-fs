...
func TestAdvisoryDomainsDoNotEnforceIO(t *testing.T) {
	c := fixture(t, DefaultConfig())
	s := session(t, c)
	o := owner(t, s, 1, 0)
	for _, command := range []storage.RangeCommand{
		record(storage.RangeExclusive, 0, 100),
		whole(storage.RangeExclusive),
	} {
		wantState(t, apply(t, s, 1, o, command), storage.Granted, "")
	}
	for _, uses := range []storage.Uses{storage.ReadData, storage.WriteData} {
		if err := c.CheckIO(background, 1, storage.UseScope{}, bytesRange(0, 100), uses); err != nil {
			t.Fatalf("advisory range enforced %v I/O: %v", uses, err)
		}
	}
}
