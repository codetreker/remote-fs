...
func TestMaterializationBoundsAndRelease(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			release, err := c.AcquireMaterialization(context.Background(), test.first)
			if err != nil {
				t.Fatal(err)
			}
			if _, err := c.AcquireMaterialization(context.Background(), test.second); !errors.Is(err, syscall.EAGAIN) {
			...
			wg.Wait()
			release, err = c.AcquireMaterialization(context.Background(), 10)
			if err != nil {
				t.Fatal(err)
			}
			release()
			if c.bytes != 0 || c.operations != 0 {
...
func TestMaterializationRejectsInvalidAndCancelledRequests(t *testing.T) {
	...
	for _, test := range []struct {
		...
		if _, err := c.AcquireMaterialization(context.Background(), test.bytes); !errors.Is(err, test.errno) {
...
func TestInvalidConfiguration(t *testing.T) {
	for _, change := range []func(*Config){
		func(c *Config) { c.MaxMaterializedBytes = 0 },
		func(c *Config) { c.MaxMaterializations = 0 },
		func(c *Config) { c.MaxFileBytes = 0 },
		func(c *Config) { c.MaxFileAttempts = 0 },
		func(c *Config) { c.FileOperationTimeout = 0 },
	} {
		config := DefaultConfig()
		change(&config)
