...
func encodeFilesystemSizeInformation(space storage.Space, full bool) ([]byte, error) {
	...
	geometry := 16
	if full {
		// Actual availability is unused virtual volume budget; caller availability
		// separately preserves any tighter limit measured by the backend.
...
// The name length describes the complete display name even on BUFFER_OVERFLOW.
