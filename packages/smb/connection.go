...
func (c *connection) process(requests []wire.Request) error {
	...
	for i, r := range requests {
		...
		if logger := c.server.config.Logger; logger != nil {
			...
			logger.Debug("SMB request completed", "component", "smb", "phase", phase, "connection_id", hex.EncodeToString(c.incarnation[:]), "session", h.SessionID, "tree", h.TreeID, "message", h.MessageID, "command", h.Command, "status", h.Status)
		}
