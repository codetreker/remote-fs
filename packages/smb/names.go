...
// Reserved device stems and trailing-dot/space rules follow the Win32 name
...
func checkWindowsLeaf(name string) error {
	if name == "" || !utf8.ValidString(name) || len(name) > storage.MaxLeafBytes || utf16Length(name) > maxNameUnits || name == "." || name == ".." || strings.HasSuffix(name, ".") || strings.HasSuffix(name, " ") {
...
func projectDirectory(entries []storage.Entry, compare nameComparer) ([]projectedName, error) {
	...
	ids := make(map[uint64]bool, len(entries))
	for i, entry := range entries {
		if err := checkWindowsLeaf(entry.Name); err != nil {
			return nil, err
		}
		if entry.Attr.ID == 0 || ids[entry.Attr.ID] || entry.Attr.Kind.Check() != nil || entry.Attr.Size < 0 {
			return nil, syscall.EIO
		}
		ids[entry.Attr.ID] = true
		projected = append(projected, projectedName{units: nameUnits(entry.Name), entry: i})
	}
	var failure error
	...
	for i := 1; i < len(projected); i++ {
		order, err := compare(projected[i-1].units, projected[i].units)
		if err != nil {
			return nil, err
		}
		if order == 0 {
...
func selectName(projected []projectedName, wanted []uint16, compare nameComparer) (int, bool, error) {
	...
	for low < high {
		mid := low + (high-low)/2
		order, err := compare(projected[mid].units, wanted)
		if err != nil {
			return 0, false, err
		}
		if order == 0 {
			return projected[mid].entry, true, nil
		}
		if order < 0 {
			low = mid + 1
		} else {
