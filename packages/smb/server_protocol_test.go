...
func authenticateProtocolAt(t *testing.T, c net.Conn, firstMessage uint64) (uint64, *signing.Session) {
	...
	if err != nil || h.Status != 0 || len(response) < 92 || binary.LittleEndian.Uint16(response[68:]) != wire.Dialect311 || binary.LittleEndian.Uint16(response[66:]) != 3 || binary.LittleEndian.Uint32(response[88:]) != 4 {
...
func TestProtocolMultiProtocolBootstrapSignsOnlyFinalTranscript(t *testing.T) {
	...
	if binary.LittleEndian.Uint16(response[68:]) != 0x02ff || binary.LittleEndian.Uint16(response[66:]) != 3 || binary.LittleEndian.Uint16(response[120:]) != 128 || binary.LittleEndian.Uint16(response[122:]) != 0 || binary.LittleEndian.Uint32(response[88:]) != 4 {
