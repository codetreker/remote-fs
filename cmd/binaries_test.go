...
func TestTheBinariesRefuseWhatTheyCannotDo(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{"a local store that is not there", serverBinary(t),
			[]string{"-listen", free, "-local-store", filepath.Join(t.TempDir(), "absent"), "-volume", "workspace", "-quota", "8M", "-initialize-lock-state"}, "no such file or directory"},
		{"a local store that is a file", serverBinary(t),
			[]string{"-listen", free, "-local-store", notADirectory, "-volume", "workspace", "-quota", "8M", "-initialize-lock-state"}, "not a directory"},
		{"an address already in use", serverBinary(t),
			[]string{"-listen", occupied, "-local-store", privateDirectory(t), "-volume", "workspace", "-quota", "8M", "-initialize-lock-state"}, "address already in use"},
		{"no volume at all", serverBinary(t),
			[]string{"-listen", free}, "a volume is required"},
		{"several volumes at once", serverBinary(t),
		...
		{"a blob container with nowhere to keep its tree", serverBinary(t),
			[]string{"-listen", free, "-blob-container", "somewhere", "-volume", "w"},
			"-metastore is required"},
		{"a blob container with no volume named in it", serverBinary(t),
		...
		{"an allowance that is not a size", serverBinary(t),
			[]string{"-listen", free, "-local-store", root, "-volume", "workspace", "-quota", "banana"}, "is not a size"},
		{"an allowance spelled as a power of 1000", serverBinary(t),
			[]string{"-listen", free, "-local-store", root, "-volume", "workspace", "-quota", "5MB"}, "power of 1000"},
		{"an allowance below the smallest there is", serverBinary(t),
			[]string{"-listen", free, "-local-store", root, "-volume", "workspace", "-quota", "1K"}, "smallest allowance"},
		{"a local store without an allowance", serverBinary(t),
			[]string{"-listen", free, "-local-store", root, "-volume", "workspace"}, "-quota is required"},
