...
func TestHandlerRetirementReclaimsDetachedFilesBeforeStorageClose(t *testing.T) {
	...
	file, err := session.OpenFile(t.Context(), "held", storage.FileOpenOptions{OpenAccess: storage.OpenAccess{Read: true, Write: true, Create: true}, InitialMetadata: map[string][]byte{"test.attribute": {0x60, 0x00}}})
	if err != nil {
		t.Fatal(err)
	}
