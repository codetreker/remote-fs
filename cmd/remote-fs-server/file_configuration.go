...
type fileBackendOptions struct {
	retained int
	service  storage.FileServiceOptions
}

func defaultFileBackendOptions() fileBackendOptions {
	return fileBackendOptions{retained: sqlite.DefaultMaxRetainedFiles, service: storage.DefaultFileServiceOptions()}
}
...
func bindFileOptions(flags *flag.FlagSet, backend *fileBackendOptions, files *httprest.FileLimits) {
	...
	flags.Var((*fileByteLimit)(&backend.service.MaxFileBytes), "max-file-size",
	...
	flags.Var((*fileByteLimit)(&backend.service.MaxMaterializedBytes), "max-file-staging-bytes",
		"maximum aggregate bytes held while applying range writes and truncation, as SIZE;\n"+
			"must cover twice -max-file-size")
	flags.DurationVar(&backend.service.FileOperationTimeout, "file-operation-timeout", backend.service.FileOperationTimeout,
	...
	flags.IntVar(&files.Session.MaxActions, "file-session-max-actions", files.Session.MaxActions,
		"maximum retained action records requested by one file session")
	flags.IntVar(&files.Session.MaxPendingActions, "file-session-max-pending-actions", files.Session.MaxPendingActions,
		"maximum pending actions requested by one file session")
...
func validateFileOptions(config commandConfig) error {
	...
	case backend.service.MaxMaterializedBytes == math.MaxInt64:
		return errors.New("-max-file-staging-bytes must be below the largest integer")
	case backend.service.MaxFileBytes > backend.service.MaxMaterializedBytes/2:
		return errors.New("-max-file-staging-bytes must cover twice -max-file-size")
	case backend.service.FileOperationTimeout <= 0:
	...
	case files.Session.MaxActions <= 0:
		return errors.New("-file-session-max-actions must be positive")
	case files.Session.MaxPendingActions <= 0:
		return errors.New("-file-session-max-pending-actions must be positive")
	...
	if err := backend.service.Check(); err != nil {
		return err
	}
	if err := files.Check(); err != nil {
		return err
	}
	if config.local.given() && backend.service.MaxFileBytes > config.local.objects.MaxObjectBytes {
	...
	if config.blob.given() {
		...
		if backend.service.MaxFileBytes > maxObject {
			return fmt.Errorf("-max-file-size is %d, above the blob object bound of %d", backend.service.MaxFileBytes, maxObject)
		}
	}
	return nil
}
